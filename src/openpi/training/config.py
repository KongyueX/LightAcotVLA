...
class DataConfig:
    ...
    # Optional Stage C Action-CoT skip-label settings.
...
class TrainConfig:
    ...
    # Optional Stage C Action-CoT skip-label settings. These are copied into
    # DataConfig at data-loader construction time to avoid changing every data
    # config factory.
    action_cot_entropy_dir: str | None = None
    action_cot_label_max_segments: int = 5
    action_cot_max_skip_segments: int | None = None
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        weight_loader=weight_loaders.ACOTCheckpointWeightLoader(
            "/root/autodl-tmp/acotvla/openpi-cache/openpi-assets/checkpoints/pi05_base/params"
        ),
    ...
    # Phase-2 long-chunk extension. Start from the validation-best H15 model
    # and use the same checkpoint as a frozen teacher for the first 15 final
    # actions. This keeps H20 opt-in and leaves every existing H10/H15 config
    # and evaluation default unchanged.
    TrainConfig(
        name="acot_libero_long_chunk_h20",
        model=acot_vla.ACOTConfig(
            coarse_action_horizon=15,
            action_horizon=20,
            pi05=True,
        ...
        ema_decay=0.999,
        weight_loader=weight_loaders.CheckpointWeightLoader(
            "/root/autodl-tmp/acotvla/checkpoints/acot_libero_long_chunk_h15/"
            "h15_lr1e5_s7_d38bf69/4500/params"
        ),
        prefix_retention_loss_weight=1.0,
        prefix_retention_horizon=15,
        prefix_retention_teacher_weight_loader=weight_loaders.CheckpointWeightLoader(
            "/root/autodl-tmp/acotvla/checkpoints/acot_libero_long_chunk_h15/"
            "h15_lr1e5_s7_d38bf69/4500/params"
        ),
    ...
    # Causal control for the Task8/9 targeted-SFT pilot. Keep the initialization,
    # optimizer, trainable parameters, and evaluation protocol unchanged while
    # reverting the data distribution to ordinary uniform all-frame replay.
    TrainConfig(
        name="acot_libero_task89_uniform_control",
        model=acot_vla.ACOTConfig(
            coarse_action_horizon=15,
            action_horizon=10,
            pi05=True,
    ...
    # One-round hard-state DAgger-style pilot. The correction dataset contains
    # only high-confidence successful continuations from snapshots visited by
    # the frozen 50999 student. Keep most samples on the original LIBERO replay
    # distribution and update only the fine action expert at a conservative LR.
    TrainConfig(
        name="acot_libero_task89_hard_state_dagger_round1",
        model=acot_vla.ACOTConfig(
            coarse_action_horizon=15,
            action_horizon=10,
            pi05=True,
...
TrainConfig(
    ...
    data=LerobotACOTGo2DataConfig(
        default_prompt="Clean the desktop.",
        repo_id="/root/autodl-tmp/acotvla/datasets/agibot/Reasoning2Action-Sim/dataset_without_depth/clean_the_desktop_part_1",
        assets=AssetsConfig(
            assets_dir=None,
            asset_id="/root/autodl-tmp/acotvla/assets/acot_autodl_pi05_debug",
        ),
        base_config=DataConfig(
...
TrainConfig(
    ...
    data=LerobotACOTGo2DataConfig(
        ...
        assets=AssetsConfig(
            assets_dir=None,
            asset_id="/root/autodl-tmp/acotvla/assets/acot_autodl_agibot_50k",
        ),
        base_config=DataConfig(
            dataloader_sampler="subtask",
            prompt_from_hl_instruction=True,
...
TrainConfig(
    name="acot_autodl_agibot_50k_bs8_clean",
    ...
    data=LerobotACOTGo2DataConfig(
        ...
        assets=AssetsConfig(
            assets_dir=None,
            asset_id="/root/autodl-tmp/acotvla/assets/acot_autodl_agibot_50k_bs8_clean",
        ),
        base_config=DataConfig(
            dataloader_sampler="subtask",
            prompt_from_hl_instruction=True,
