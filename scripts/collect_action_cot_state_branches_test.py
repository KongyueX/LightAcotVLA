...
def test_branch_actions_preserve_protocol_and_shape() -> None:
    ...
    expected_fault = -np.where(primary[1:3, 6] >= 0.0, 1.0, -1.0)
    np.testing.assert_array_equal(branches[5][1:3, 6], expected_fault)
    assert np.all(branches[5][1:3, 6] != np.sign(primary[1:3, 6]))
