...
2026-07-24 16:37（Asia/Shanghai）审计从项目README和GitHub Contributors中移除`LinqingZhong`个人影响的可行性，尚未改写README、提交历史或远端。GitHub当前将仓库识别为非fork独立仓库；`main`共99个线性提交，最早连续5个提交由`zhonglinqing`创建，其中3个以Agibot邮箱显示为匿名、2个以Gmail映射到`LinqingZhong`账号，因此Contributors API显示`KongyueX=94`、匿名贡献3、`LinqingZhong=2`。这5个提交从根提交开始，任何移除或改作者操作都会重写之后全部99个SHA；当前实验日志中也有大量历史commit引用会因此失效。

README中的个人姓名只出现在末尾BibTeX作者列表，但顶部仍有上游论文/项目badge、`research fork`表述、上游链接和由初始提交引入且未修改的`docs/framework.png`。当前260个跟踪文件中有135个路径来自初始提交，112个blob仍与root版本完全相同，因此可以降低README中的个人展示并把必要来源移入第三方声明，但不能据此声称技术或著作来源已完全独立。若必须让GitHub Contributor账号消失，较诚实的做法是为当前快照创建新的orphan根提交、在第三方声明中保留Apache-2.0来源边界后以`--force-with-lease`替换`main`；远端目前无其他branch、tag、PR、fork、star或其他collaborator，协作成本较低，但该破坏性操作仍需用户明确确认，且GitHub统计刷新可能需要约24小时。

2026-07-24 16:42（Asia/Shanghai）根据用户希望保留自己的提交历史，进一步收敛为“替换上游根历史并重放用户提交”的方案，尚未执行历史改写。计划将最早5个上游提交压成一个中性的初始导入根提交，再把其后的94个用户提交按原顺序重放；这样能保留每个用户提交的内容、作者、作者时间和提交说明，并让默认分支不再包含原5个个人提交。由于Git提交SHA包含父提交哈希，所有重放后的用户提交都会获得新SHA，无法同时保留旧SHA；执行前仍需创建仓库外Git bundle备份、锁定远端旧`main`作为`--force-with-lease`保护值，并得到用户对强制改写`main`的明确确认。

2026-07-24 16:51（Asia/Shanghai）解释审计中发现的16个个人目录路径，未修改配置或运行实验。它们均位于`src/openpi/training/config.py`，是以`/mnt/public/zhonglinqing/data/datasets/`开头的上游服务器绝对数据路径，不是账号凭据或GitHub Contributor来源：其中10条属于VLABench primitive LeRobot数据集，1条属于LIBERO-Plus，3条属于GO1 pick/wipe/pour数据，1条属于AgileX pick数据，1条是ICRA simulation challenge normalization assets。只有选用对应旧训练配置时才会参与路径解析，当前LIBERO V2-P、Task8/9 DAgger和IR-ACoT实验路径不依赖这些值；后续可将其改为环境变量或用户传入的数据根目录，提高可移植性并移除个人机器痕迹。

2026-07-24（Asia/Shanghai）基于IR-ACoT快速否证结果完成同类论文检索与下一方法方向分析，本任务未修改模型代码或运行新实验。检索重点覆盖少步动作生成、VLA固定成本加速、慢推理/快控制双系统、具身CoT训练与因果faithfulness；主要一手参考包括HiRT（arXiv:2410.05273）、VLA-Cache（2502.02175）、EfficientVLA（2506.10100）、FiS-VLA（2506.01953）、LaST0（2601.05248）、ECoT-Lite（2505.08243）、ERVLA（2606.03784）、DeepThinkVLA（2511.15669）、Coarse-to-Control（2606.07107）和One-Step Flow Policy（2603.12480）。

文献与本项目证据共同表明，继续只缩短coarse/final flow NFE最多优化action head，不能突破当前原10/10为95.844 ms、B6 1/1仍为68.929 ms的VLM/prefix/IAR固定成本上限；单纯token cache、动态层数、通用compute scheduler、reasoning dropout或one-step flow也已有直接先例，单独采用不足以形成新的ICRA级贡献。更合适的主线是ACoT特有的事件触发多速率架构：低频完整VLM+EAR+IAR生成并缓存可解释Action-CoT计划，高频轻量执行器每个控制周期读取最新视觉/本体状态，对缓存计划进行phase alignment与residual action correction；只有当观测与计划不一致、接触/夹爪/阶段事件发生或fresh-vs-stale短期regret变高时才刷新完整推理。它与`h_exec`不同，高频分支仍逐步闭环生成新动作，而不是从1–10中选择开环执行长度。

该方向的主要新颖性风险来自HiRT、FiS-VLA、LaST0、AC2-VLA和ElegantVLA，因此“slow-fast/cache/scheduler”本身不能作为核心claim。若继续，应把贡献限定为Action-CoT结构化因果记忆、基于计划有效性的反事实刷新、最新观测残差纠正，以及necessity/sufficiency/mediation闭环评测。建议先做固定1:4的最小可行baseline：冻结原完整ACoT慢分支，用连续轨迹和随机cache age训练小型plan-conditioned快分支，不加RL和动态gate；只有fast-path真实延迟明显低于当前68.929 ms固定路径、stale-plan open-loop误差可控并在小规模闭环不明显退化后，再增加phase tracker与learned refresh。若一步动作质量成为瓶颈，可用One-Step Flow Policy式interval self-consistency/self-guidance/warm-start替换当前简单endpoint MSE；若EAR仍被旁路，再引入interchange intervention或小型outcome-aligned RL，而不应继续放大已失败的response-MSE loss。

2026-07-24 17:22（Asia/Shanghai）按用户明确授权完成GitHub `main`根历史替换，同时保留其后的94个用户提交。执行前将旧`main=f23c6c272413a789d317a64257d3907afffa75e6`、本地分支和未提交日志完整备份为仓库外bundle `/Users/kongyue/LightAcotVLA-history-backups/2026-07-24-1652-pre-history-rewrite.bundle`并通过`git bundle verify`。最早5个个人上游提交被一个新根`1aca813771d663d5f01f8babdabdb8295f2646df`替换；94个后续提交逐对象重建，逐项核对每个提交的tree、author、committer、author/committer timestamp和完整message均为0差异，仅因父链变化生成新SHA。随后以提交`bce6f1b4a3e53decba675148923bb88e3b720de4`重写README为LightAcotVLA主体、加入中性`THIRD_PARTY_NOTICES.md`并删除旧`docs/framework.png`，没有修改模型或训练配置。

远端更新使用锁定旧SHA的`--force-with-lease`成功完成，SSH和GitHub API均确认`main=bce6f1b4a3e53decba675148923bb88e3b720de4`；GitHub Contributors API已即时只显示`KongyueX=96`，不再显示原个人账号或匿名旧作者。重写后历史为96个提交：1个新根、94个原样重放提交和1个项目身份清理提交。`src/openpi/training/config.py`的SHA-256仍为`40d21eafe6db3e3de3867e9ffbb8b0a84fe0ee1729cd302dbb85e2b82b0b2840`，16个服务器匹配绝对路径及本地`assets`、`checkpoints`符号链接均保持不变；另一并行任务的`policy.py`和multirate文件改动也原样保留为未提交工作树内容，未被本次操作暂存或推送。

2026-07-24（Asia/Shanghai）完成固定1:4多速率ACoT的2k快速可行性验证，代码实现与公平性修复对应提交`0616d92`至`da628a1`。方法让低频完整ACoT每4次policy call刷新并缓存EAR `[15,32]`与IAR `[18,1024]`，高频小执行器每次读取最新两视角64×64图像、state、cached EAR/IAR和cache age输出动作；V2进一步保留4×4视觉空间token、对IAR做attention，并将cached plan限制为调制observation-conditioned residual，SPRD只在训练期以fresh phase作为辅助标签。相关单元测试14项通过，ruff与格式检查通过，未改变原eval默认路径。

数据导出位于服务器`/root/autodl-tmp/acotvla/multirate_acot_pilot/windows2k`：跨LIBERO-10共选取并成功生成2000/2000个连续窗口，0失败，125个HDF5 shard约411 MiB；teacher每窗口4次调用，均值85.2023 ms、p95 85.6229 ms。按task分层且episode-disjoint得到1598 train、194 validation、208 test窗口，共832个test frame，其中624个为age 1–3 stale frame、52个为真实gripper transition frame；train-only动态输出范围在test上无饱和。

离线7D action MSE在age 1–3 stale frame上的结果为：B6一步参考0.08297185、无网络Hold4基线0.11547171、V1执行器0.12333240、V2关闭SPRD为0.12478414、V2开启SPRD为0.12559691，越低越好；age3分别为0.06961771、0.14436063、0.12601627、0.12810315、0.12839602。真实gripper transition sign accuracy为B6 73.08%、Hold4 53.85%、V1/V2均57.69%。V1使用fresh plan的oracle为0.07934690，说明若计划及时刷新，执行器容量并非完全不足，但当前stale-plan correction没有学成。

SPRD将fresh-phase辅助目标在stale frame上的MSE从未刷新的0.09030824降到0.08977419，只改善0.59%，远未达到预设至少20%的门槛；其action MSE还比关闭SPRD的matched V2略差。V1、V2无SPRD和V2有SPRD均未击败Hold4或B6，全部触发`offline_gate_pass=false`与`immediate_stop=true`，因此按预注册规则没有启动LIBERO闭环，当前没有可报告的成功率。

最终V2 gated executor含1,781,728参数，在GPU上50次warmup、2000次计时得到mean 0.6441 ms、p95 0.7017 ms；以full ACoT 95.844 ms和B6 68.929 ms为参考，固定1:4摊销为24.4441 ms，理论加速分别为3.921倍和2.820倍，速度门槛通过。结论为速度假设成立但精度假设失败：不应在同一2k数据上继续搜索SPRD权重或单纯扩大执行器；下一步需要改变slow-plan刷新机制、加入真正的hard-state corrective数据，或重新设计可观测的plan-validity/event-trigger refresh，再单独验证闭环。精简结果归档于`results/multirate_acot_pilot/2026-07-24/summary.json`。服务器数据盘为350 GiB，当前使用214 GiB、剩余137 GiB，本轮不需要继续扩容。

2026-07-24 19:12（Asia/Shanghai）实时核对固定1:4多速率ACoT实验监督状态：服务器没有tmux session、匹配的训练/导出/LIBERO评测/服务进程或GPU compute进程，说明本轮任务均已正常退出而非意外卡住。V1、V2无SPRD、V2有SPRD和最终V2测速四份summary均存在，最后一份测速结果于19:12写入；数据盘仍为350 GiB、使用214 GiB、剩余137 GiB。当前无需继续进程监督；若启动下一条方法路线，应创建独立tmux、日志与阶段性离线门槛后再恢复监控。

2026-07-24（Asia/Shanghai）再次实时核对固定1:4多速率ACoT实验状态，结论未变化：服务器无tmux、匹配实验进程或GPU compute进程，V1、V2无SPRD、V2有SPRD及最终测速四份summary均完整存在；数据盘仍使用214/350 GiB、剩余137 GiB。本轮验证进度为100%，离线速度门槛通过但精度门槛失败，按规则未启动闭环。

2026-07-24（Asia/Shanghai）在不重新训练、未修改实验代码的前提下，对固定1:4多速率ACoT的2k HDF5做了只读可辨识性和结构上限审计。发现当前export以`window_seed=policy_seed+anchor`为每个重叠window生成teacher cache/target，8000个frame occurrence只有7093个唯一物理frame；1724个occurrence位于重复组，占21.55%，这些重复帧的图像和state相同但seed全部不同，teacher action与fresh phase的组内pairwise MSE分别约0.3575和0.3507。cached EAR携带window特定随机模式，因此当前损失客观上更容易通过复制cache解释标签，而当前观测无法解释同一物理帧的seed-dependent target。

结构oracle显示过期EAR的主要误差来自时间相位而非纯动作残差：episode-held-out的624个stale test frame上，固定age/2对fresh phase的MSE为0.090308；在cached EAR上逐样本搜索连续phase并以全部7维fresh phase选择位置后可降至0.016017，gripper sign accuracy为99.36%，中位phase修正约0.475个EAR token。若只按连续6维选phase，fresh MSE为0.043659，说明gripper/contact事件必须作为phase landmark或独立事件监督。全计划age-aligned MSE为0.036902，train PCA basis的oracle rank-8/16 revision分别为0.024551/0.018230，但这些都使用了当前fresh target选择oracle参数，只证明表示容量，不能当作可部署结果。

现有输入下的可预测性仍然不足。episode-held-out线性probe预测fresh-phase correction时，plan+age为0.07127、加入state为0.07069、加入4×4 current/anchor图像差分的组合为0.07393；观测没有稳定增益。进一步按`(task,episode,absolute frame)`选择最小seed occurrence，并同时把每个window的anchor cache与current target替换为canonical label后，只读重跑得到fresh phase：stale 0.09057、fixed-age warp 0.08121、plan-only ridge 0.07229、加state 0.07250、加anchor/current RGB delta 0.07404；teacher action对应为0.13489、0.12950、0.10578、0.10602、0.11274。消除跨window seed冲突后观测增益仍为负，说明seed冲突会强化cache捷径，但根因还包括2k数据全是expert nominal state、没有相同cache age下不同真实进度或stale-policy诱发的corrective state。

据此将下一候选从直接action residual改为结构化Action-CoT状态运输，暂称`PC-ACoT / Transported Action-CoT`。slow ACoT缓存完整EAR、压缩IAR、anchor observation/state；高频分支先根据已执行动作和elapsed time对EAR做连续offset/speed warp，再用当前观测innovation修订整条显式计划与可选compressed-IAR slots，更新后的状态写回下一控制步，plan-only小decoder从revised EAR产生动作，从结构上禁止绕过EAR直接做observation-to-action residual。第一版应先只验证EAR phase/事件transport，不立即加入IAR recurrence或RL；只有phase版本通过离线门槛后再增加low-rank geometry/IAR posterior correction。刷新调度只作为安全fallback而非主创新。

文献查重表明，泛化的slow-fast、cache update、recurrent memory和learned refresh scheduler已分别被HiRT、FiS-VLA、LaST0、VLA-Cache、A2C2、EvoScene-VLA、μVLA、AC2-VLA和ElegantVLA覆盖；可谨慎主张的差异只能落在ACoT特有的explicit EAR连续时间运输、事件对齐、可写回Action-CoT state及延迟slow-result replay。下一实验前必须先修复frame-consistent teacher seed/标签协议，并构造action hold/drop、speed variation、gripper timing与小扰动的branched hard-state数据，使相同cache age对应不同真实progress；否则任何更强视觉网络仍会继续学plan+age shortcut。激进停止线建议为4-step action MSE至少达到0.112335（关闭V1到fresh-oracle gap的25%），值得扩成论文主线需达到约0.10134（关闭50%）且保持至少3倍wall-clock加速；同延迟A2C2若持平，则停止完整cache updater路线。

2026-07-24（Asia/Shanghai）实时核对PC-ACoT后续状态：当前服务器没有tmux、相关训练/导出/评测进程或GPU compute进程，说明新一轮PC-ACoT代码实现、branched hard-state导出和训练尚未启动；已经完成的是旧2k数据的只读可辨识性/oracle/canonical-label诊断与下一算法设计。数据盘仍剩137 GiB，空间足以开展下一轮。下一项实际工程工作应先修复frame-consistent teacher seed和正式诊断脚本，再实现EAR transport与branched collector，而不是直接复用当前有冲突的2k标签启动训练。

2026-07-24（Asia/Shanghai）完成Transported Action-CoT / PC-ACoT phase-transport首轮快速训练与止损验证，代码提交`ad66b2e`、`c1c5fc5`和`16a5755`已推送GitHub main并同步到隔离服务器`/root/ACoT-VLA-multirate`。新增`src/openpi/models/transported_action_cot.py`、对应9项单元测试和`scripts/train_transport_acot.py`；服务器单测9/9通过。模型含474530参数，共享CNN读取anchor/current视觉差、state差、cached EAR/IAR与elapsed age，预测连续EAR start phase与speed并运输完整EAR；action decoder只读取运输后的前三个EAR token，当前观测没有直接到action的旁路。训练按task分层、episode-disjoint复用2000个四帧window，拆分为1598/194/208；构造elapsed age `d=1..3`、physical progress `p∈{d-1,d,d+1}∩[0,3]`的same-window time-warp监督，checkpoint按validation nominal action与time-warp phase两个相对基线的等权无量纲分数选择，并在最终评测加入`current=anchor`消融。

首个随机合法progress训练请求800步、在step600早停，最佳step200、训练49.09秒；test nominal action MSE为0.12354810，all-time-warp action MSE为0.12100652、phase MAE为0.23035288、完整EAR MSE为0.03132644。为排除nominal比例不足，随后固定1:1 nominal/non-nominal采样再训练800步，最佳step500、训练54.02秒；test nominal action MSE为0.12410770，未改善首轮。该balanced run在all-time-warp test上action MSE为0.11922247、phase MAE为0.22025484、完整EAR MSE为0.03096416，相对fixed-age分别改善15.72%、29.52%和22.43%；将current image/state替换为anchor后action MSE恶化到0.16842693、phase MAE到0.72203183，支持模型确实使用了最新观测而非只学plan+age均值。

主动作门槛失败：balanced nominal action MSE 0.12410770高于预注册0.112335，也差于Hold4 0.11547171、B6 0.08297185和历史V1 0.12333240；full-EAR相对fixed-age只改善22.43%，未达到50%门槛。phase MAE≤0.25通过，但phase-only transport无法同时保持nominal fidelity，且1:1重采样不能解释失败，因此按规则不补seed、不跑LIBERO闭环，也不能声称成功率提升。GPU batch1、50次warmup和2000次计时得到fast path mean 0.28664 ms、p95 0.32029 ms；按full ACoT 95.844 ms计算1:4摊销24.17598 ms、理论3.9644倍加速，速度假设再次通过。服务器实验目录总计仅7.3 MiB，数据盘仍剩137 GiB，无需扩容；训练和profiling均已退出，GPU空闲。精简结果归档于`results/transported_action_cot_pilot/2026-07-24/summary.json`。

2026-07-30（Asia/Shanghai）复核Transported Action-CoT归档结果并解释当前效果，本次未重新训练或运行闭环。结论仍为“速度成立、phase运输有信号、动作质量未过线”：balanced run将time-warp phase MAE从0.3125降到0.22025、完整EAR MSE降低22.43%，且`current=anchor`消融令action MSE恶化41.27%，说明最新观测确实参与了运输；但nominal action MSE为0.12411，差于Hold4的0.11547、历史V1的0.12333及0.112335继续门槛。该版本只能作为有效的phase-tracking消融，不能作为可进入LIBERO闭环或论文主结果的最终方法；若继续，应增加事件/夹爪专门建模与非时间相位的low-rank EAR geometry correction，而不是继续调phase loss或重复增加seed。

2026-07-30（Asia/Shanghai）结合当前代码、归档结果及一手论文重新说明现阶段方法，本任务未修改模型或运行新实验。当前实现准确定位为固定1:4的`Transported Action-CoT phase-only pilot`，不是执行长度`H`选择器，也不是完整的recurrent PC-ACoT：慢分支在anchor时生成并缓存EAR `[15,32]`、IAR `[18,1024]`与anchor observation/state；47.45万参数快分支逐tick融合anchor/current视觉差、状态差、cached EAR/IAR与cache age，只预测EAR连续采样的start offset和speed，通过线性插值运输整条EAR，plan-only decoder再从前三个transported EAR token产生动作。训练用2000个四帧nominal expert windows构造1:1 nominal/non-nominal local time-warp重索引监督，损失为action Huber、full-EAR Huber与0.25倍phase MSE；这些样本不是现实扰动后的动力学分支。

论文边界核对显示：ACoT-VLA与Coarse-to-Control已覆盖action-space CoT和coarse-plan→execution；HiRT与FiS-VLA已覆盖低频VLM/高频视觉控制；VLA-Cache与Fast ECoT已覆盖跨帧token或reasoning复用；A2C2已覆盖latest-observation、time-aware轻量head对旧action chunk的逐步修正；RTC覆盖异步chunk生成/执行；ECoT-Lite覆盖训练时reasoning supervision而测试时去掉显式reasoning；μVLA覆盖一般recurrent memory token。因此当前可区分点只能收窄为“通过plan-only因果瓶颈对显式完整EAR做ACoT-specific temporal transport”，而phase-only版本尚未实现event alignment、low-rank geometry correction、IAR posterior update、plan-state writeback、learned refresh或异步执行。现有结果说明这一窄机制能估计执行进度，但两个时间自由度无法修正计划内容与离散夹爪事件；后续若扩展，A2C2必须作为同延迟matched baseline。

2026-07-30（Asia/Shanghai）在`ssh -p 30803 root@connect.westb.seetacloud.com`完成event-factorized Action-CoT的快速go/no-go训练与验证。提交`972e49d`在`TransportedActionCoTExecutor`中增加仅129参数的标量事件头：连续6维仍由普通transported EAR解码，事件头只给cached EAR夹爪通道增加有界`[-2,2]`相位偏移；`enable_event_shift=false`提供同参数树、同loss的严格`δ=0`对照，旧`phase/plan/direct`默认行为保持不变。服务器针对参数量、零初始化匹配、只改夹爪以及有限梯度运行4项定向测试，4/4通过；代码与结果分别以`972e49d`和`7353ba0`推送GitHub main。

无训练oracle先确认该表示有上限：固定split7的624个test nominal pairs中，`δ=0`可完整匹配422条目标夹爪序列（67.63%），单一连续`δ∈[-2,2]`可匹配606条（97.12%），平均token sign accuracy为99.754%；目标EAR中378条无事件、229条单事件、17条多事件。把phase模型动作夹爪维直接替换为GT后，seed7/17/27的nominal 7D MSE理论下界分别为0.056263、0.057011和0.056778，明显低于0.112335门槛，因此没有在训练前否决事件路线。

按用户要求缩短验证，只用固定split/model seed7各训练600步，不启动完整分支或三种子矩阵；两条训练均使用action-based checkpoint selection、strict event gradient isolation、0.05 gripper-state/action-gripper/event loss和0.001无事件offset正则，profiling从2000次缩至200次。`event_zero`最佳step500、训练45.78秒，test nominal/all-warp action MSE为0.12322478/0.12045953，all-warp event F1为0.44046、phase MAE为0.24821；`event_learned`最佳step400、训练26.59秒，对应MSE为0.12186678/0.11860245，分别相对改善1.10%/1.54%，all-warp event F1升至0.48821，但phase MAE为0.25058。分层上learned offset对progress offset `-1/0`的MSE分别改善0.004274/0.001358，对`+1`反而恶化0.001019，说明其主要学到单向滞后补偿，尚非稳健的事件修正。

当前结论为方向性信号存在但总门槛失败：learned event shift未达到nominal MSE≤0.112335，all-warp phase MAE也以约0.00058擦线失败，因此按快速止损规则不补第二seed、不运行LIBERO闭环，不能声称成功率提升。两分支GPU p95约0.40–0.42 ms、理论固定1:4加速约3.95倍；实验目录仅6.9 MiB，数据盘仍剩137 GiB，无需扩容。精简结果归档于`results/transported_action_cot_event/2026-07-30/{README.md,summary.json}`，服务器已无tmux、训练进程或GPU compute进程。下一候选应允许在置信度控制下新增、抑制或翻转夹爪事件并保留`δ=0` fallback，而不是继续扩大单标量offset训练。

2026-07-30（Asia/Shanghai）基于phase、full-plan、isolated-event与scalar event-shift的现有证据分析替代路线，本任务未修改模型代码或运行新训练。event-shift不应被判为完全无效：其oracle覆盖97.12%夹爪序列且learned分支带来约1%–1.5%动作MSE改善，适合作为后续模型的时间归纳偏置；但单一全局偏移不能创建、删除或翻转事件，也不能改变连续6维几何。更重要的是，现有2k数据全部来自成功专家四帧窗口和合成progress重排，没有同cache age下的真实偏航、动作延迟、抓取失败与恢复分支，且历史审计发现约21.55% frame occurrence为同物理帧不同teacher seed的冲突标签，因此继续扩大预测头很可能仍只学习平均修正。

推荐下一主线为`Recurrent Action-CoT State Updater / Branched Plan-State Correction`：把`S_t=(EAR_t,IAR_t)`作为逐控制步更新的显式推理状态，使用最新观测、state、已执行动作和上一推理状态计算`S_t=Uθ(S_{t-1},o_t,a_{t-1})`。EAR更新器采用one-step conditional residual flow逐token修订计划，输出单调时间映射、连续6维spline/low-rank几何修改、逐token gripper keep/open/close事件编辑及置信度；action decoder只能读取更新后的EAR/IAR，不允许observation-to-action旁路。该定义在算法上是增量维护Action-CoT，而不是预测`H/K/δ`或放大action residual head；scalar event-shift仅作为初始化或一个可解释分量。

可信的第一轮数据应从同一MuJoCo snapshot用固定canonical teacher seed分支：nominal、hold/drop、动作缩放、夹爪早/晚、小位姿扰动和失败策略动作在相同cache age下形成不同真实状态，再由fresh 10/10 teacher记录目标EAR/IAR/action与outcome。最快go/no-go可先收约150个root、每个4个branch，先做warp+spline+event无训练oracle；oracle若不能关闭至少50%的stale-to-fresh EAR gap则停止。通过后只训seed7约400–600步，必须同时优于历史phase、full-plan、matched direct与Hold4，nominal MSE达到0.112335，并在held-out disturbed branches上关闭至少25%的action gap；全过后才做20个paired hard-root闭环，不先跑多种子矩阵。备选路线依次为fresh Fast-EAR加prefix/KV cache，以及仅作安全fallback的plan-validity gated refresh。

相关一手工作给出边界：A2C2按最新观测和chunk位置直接修正旧action chunk；HiRT与FiS-VLA已覆盖低频推理/高频执行双系统；RTC、training-time action conditioning和REMAC覆盖异步chunk与prefix条件；VLA-Cache覆盖视觉token/KV复用；One-Step Flow Policy支持用self-consistency和warm start实现单步flow；ECoT-Lite与ERVLA则把CoT降为训练期representation supervision。因此若投稿，贡献必须落在“可观察、可干预、逐步更新的action-space CoT state”及其branched counterfactual监督，而不能只主张slow-fast、cache、direct chunk correction或reasoning dropout。

2026-07-30（Asia/Shanghai）审计Recurrent Action-CoT State Updater候选的四项输出设计依据，本任务未修改模型代码或运行训练。结论是四项证据强弱不等：单调时间映射直接对应现有phase-transport已观察到的22.43% full-EAR误差改善，逐token keep/open/close事件编辑对应scalar event-shift 97.12%的表示oracle覆盖及约1%–1.5%的learned MSE信号；连续6维低维样条/固定basis几何修正得到movement primitive与B-spline trajectory representation支持，但具体4个控制点只是待验证的容量假设；逐token置信度主要是安全回退与选择性更新机制，当前项目没有直接实验支持，第一版不应把它作为核心更新分量。

为避免把任意维度包装成理论结论，建议第一版采用嵌套消融：现有transport作为base，依次加入单调时间warp、离散事件编辑、再加入固定basis的连续几何残差；置信度延后到编辑器本身有效之后，并先从plan-level verifier或完整risk-coverage曲线开始。EAR长度15与连续动作6维来自现有表示，事件3类来自最小编辑操作keep/open/close；时间knot数和几何control-point数均应先在held-out branched stale/fresh pairs上计算representation oracle gap closure，再用相同数据、encoder和预算选择最小非Pareto支配结构。还需约束时间warp、沿轨迹切向几何残差与事件边沿之间的可辨识性，并保证所有新增head零初始化时精确回退当前transported EAR。

2026-07-30（Asia/Shanghai）完成plan-level Action-CoT update confidence的快速go/no-go验证。提交`061ecbf`至`c19e163`新增独立129参数`ActionCoTUpdateConfidence` sidecar、同checkpoint的raw base/proposal输出、严格episode-held-out训练评估脚本和单元测试；服务器ruff检查通过，`transported_action_cot_test.py`共32项通过。Router读取frozen event editor的stopped-gradient 128维context，只预测完整proposal相对未修改phase-transport base是否值得采用；主editor、视觉编码器和proposal不受router loss影响，拒绝时整计划原子回退，不做逐token软混合。

固定`event_learned` checkpoint的held-out all-warp test共有1664对，always-base、always-update和逐样本oracle route的7D action MSE分别为0.12306442、0.11860245和0.10837799，说明选择有真实oracle上限。训练集12784对中5051对超过1%相对risk margin，其中3050 accept、2001 reject；validation选择阈值0.475599、覆盖率59.99%。但test AUROC仅0.54262，覆盖率63.10%时learned routed action MSE为0.12050465，差于always-update 0.11860245；accepted regression rate为47.33%，learned route只关闭4.77%的oracle risk gap，预注册offline gate失败。该结果与额外只读probe一致：现有nominal/synthetic local-time-warp 2k数据无法可靠识别有害事件修正，因此不继续搜索q阈值、hidden size或token-level q。

速度方面，frozen editor mean/p95为0.40323/0.41669 ms，editor+router为0.42651/0.43587 ms，router平均增量0.02328 ms；固定1:4理论摊销24.28088 ms，相对95.844 ms full ACoT为3.9473倍。实验目录仅84 KiB，数据盘仍剩137 GiB。完整结果归档于`results/transported_action_cot_confidence/2026-07-30/{README.md,summary.json}`。本轮完成后已通过仓库现有`scripts/watch_experiment_feishu.py --test_message`成功发送飞书结果通知，命令退出码为0且未输出凭据。下一轮应优先构造固定canonical teacher seed的branched disturbed snapshot数据，再验证structured event/geometry update和confidence可预测性。

2026-07-30（Asia/Shanghai）开始实现Recurrent Action-CoT下一轮的canonical branched数据链路，尚未采集正式数据或运行训练。新增`src/openpi/action_cot/branched_dataset.py`及测试，以一条HDF5记录原子保存同一MuJoCo root的anchor图像/state、cached EAR/IAR/action，以及六个固定4-step分支的current图像/state、fresh 10/10 EAR/IAR/action、实际执行动作与有效位；分支为nominal、packet fault、0.5/1.25动作缩放、translation pulse和gripper shift，所有anchor/endpoint使用绑定物理`task/episode/step`的同一canonical policy seed，后续split必须按root或episode而非branch。

实现过程中只读实测发现旧`collect_execution_horizon_counterfactuals._capture_snapshot`只恢复MuJoCo physics、wrapper scalar和RNG，没有恢复robosuite OSC controller goal、robot/controller torques、gripper current action及`sim.data.ctrl`；旧restore从同一root重复执行相同4动作时会出现约1.29–2.09 mm EEF漂移及像素变化，不能作为canonical counterfactual。新`scripts/collect_action_cot_state_branches.py`因此使用扩展快照，在重新生成observation前恢复这些控制器状态并设置`controller.new_update=True`，且每个root写入前强制重复相同4-action endpoint至少3次，要求physics/EFF误差不超过`1e-10`且双视角图像逐像素一致，否则拒绝该root。当前仅完成本地语法编译与`git diff --check`；本地pytest环境缺少`pynvml`，尚需在服务器环境运行ruff、单元测试和真实LIBERO determinism pilot。服务器实时剩余137 GiB且GPU空闲，本轮预计数据远低于1 GiB，不需要扩容。

2026-07-30（Asia/Shanghai）完成canonical branched数据链路的首个跨任务10-root pilot与只读表示oracle。实现提交`991a822`至`cd0513f`后，服务器ruff通过，branched dataset/collector共4项定向测试通过；policy server使用50999、10/10 Action-CoT和port8042。10个root分别来自LIBERO-10十个task的episode0、decision step10，每root六个分支均实际执行4步并得到fresh EAR/IAR/action；三次重放的最大physics误差为`4.0442e-14`，EEF误差和双视角像素误差均为0，验证补全controller状态后的snapshot协议达到预设determinism gate。pilot约4 MiB，数据盘仍剩137 GiB。

以固定4 env steps、coarse stride2对应的phase2为基线，逐样本scalar phase oracle在连续6D EAR上对nominal、packet fault、scale down、scale up和translation pulse分别关闭`51.77%/62.86%/73.13%/39.72%/54.61%`误差；affine相对scalar只再增加约`0.8–3.2`个百分点，3-knot相对affine只增加约`1–2.4`个百分点，故暂不扩大复杂warp。packet、scale down、scale up相对nominal的最优phase平均变化为`-0.168/-0.359/+0.159` token，且分别有9/10、10/10、9/10 roots方向一致，支持真实状态进度信号；但train/test尚不存在，不能当作learned结果。一个pooled全局phase约`1.1781`已将6D MSE从`0.046444`降至`0.025251`、关闭45.63%，因此下一learned updater必须超过train-only全局校准baseline，不能只和固定phase2比较。

pilot同时发现原`gripper_shift`分支在10/10 early roots上与nominal的current physics/state/image及fresh EAR/IAR/action完全相同；扩大采集在28 roots时按快速止损停止，该目录只保留为诊断、不用于训练。提交`40b3f9a`和`8dae5f0`把它改为第1–2 tick强制反向离散夹爪命令的`gripper_fault`，单root复测的fresh EAR 7D MSE从nominal的`0.0629298`变为`0.0667271`，确认不再是重复分支；随后从新目录启动10 tasks×3 episodes×最多8 roots的扩大采集。Iteration 2 pilot结论已用现有`scripts/watch_experiment_feishu.py --test_message`成功发送飞书，命令退出码0且未输出凭据。当前结论仅为“canonical数据有效且scalar effective-progress存在oracle信号”，尚未证明predictor可学习、动作或闭环成功率提升。

2026-07-30（Asia/Shanghai）实现branched effective-progress最小可学习性probe，正式500-step结果尚未产生。新增`scripts/train_branched_effective_progress.py`及3项测试：按task分层并以episode为单位拆分root后再flatten有效branch，从cached/fresh EAR连续6维的`[0,4]` dense grid计算每样本scalar phase oracle；221713参数的共享CNN/state/executed-prefix/cached-plan encoder只输出一个有界phase并通过它运输EAR，没有observation-to-action residual或action decoder。评估固定phase0、固定age phase2、仅train集校准的全局phase、privileged branch-ID train mean、learned phase、`current=anchor`消融和dense oracle，报告6D/7D EAR MSE、phase MAE、gap closure及逐branch分层。服务器ruff/format与3项测试通过，并在真实fault HDF5上完成1-step全链路smoke约19.6秒；该smoke只验证代码和checkpoint路径，不能当作学习效果。

2026-07-30（Asia/Shanghai）完成canonical branched数据扩大采集和scalar effective-progress的三组正式go/no-go验证。数据集`/root/autodl-tmp/acotvla/action_cot_branches/20260730_canonical240_fault_8dae5f0`覆盖LIBERO-10、每任务3个episode和每root六种4-step动力学分支，共得到238个通过确定性门槛的root、1428条有效branch；2个root因重放不确定性被拒绝，拒绝率0.83%，数据约85 MiB。通过root的最大physics重放误差为`2.0167e-11`、最大EEF误差为`2.1094e-15`、像素误差为0；数据盘仍剩约137 GiB，无需扩容。提交`83a77ac`进一步把`ctrl/qacc/qacc_warmstart/qfrc_applied/xfrc_applied/mocap/userdata`等MuJoCo solver状态纳入未来snapshot恢复，提交`71889e2`加入正式probe脚本，服务器相关5项定向测试通过。

三次500-step训练分别使用seed/split 7、17和29，每次按task分层、episode-disjoint分配train/validation/test并由validation选择checkpoint，训练耗时`27.98/28.13/28.39`秒。test overall 6D EAR MSE中，learned相对train-global calibration分别改善`3.73%/3.29%/1.33%`；相对更强的train-only privileged branch-ID mean只改善`2.31%/1.62%/0.53%`，仅关闭该强基线到dense oracle剩余差距的`9.19%/6.27%/1.91%`。disturbed分支相对privileged branch mean改善也只有`2.36%/1.00%/0.63%`；将current observation替换为anchor的推理消融与learned相比只差`1.62%/0.46%/2.05%`。三组均低于预设“相对强基线至少5%、关闭剩余差距至少25%、当前观测贡献至少3%–5%”门槛，因此scalar phase updater正式判为no-go，不运行LIBERO闭环，也不把它作为ICRA主创新。独立协议审计还指出executed prefix可暴露固定扰动类型、`current=anchor`属于OOD消融且7D oracle并未优化夹爪维；后续验证需使用matched observation ablation、未见扰动/强度外推及独立夹爪事件指标。seed7正式结果和no-go判断已通过`scripts/watch_experiment_feishu.py --test_message`发送成功并得到`feishu_notification_verified`。下一步转向把EAR编译为带目标几何、夹爪/接触mode和termination predicate的Event-Option ACoT，由最新观测驱动轻量闭环option controller，而不再扩大scalar phase predictor。

2026-07-30（Asia/Shanghai）完成Event-Option cached-action恢复空间与三种可学习selector的快速验证。只读privileged oracle在238 roots/1428 branches上从cached final chunk的10个token中按fresh teacher连续6维选择最优token：overall raw environment-action MSE由fixed index4的`0.00424958`降至`0.00109673`，关闭`74.19%`；disturbed由`0.00443412`降至`0.00115056`，关闭`74.05%`，且最优index覆盖0–9、index4仅占`13.80%`。但7D总体只关闭`35.55%`，translation pulse分支为`-6.99%`，因此连续进度与夹爪事件必须分离；该oracle只证明cached chunk包含恢复候选，不是deployable性能。

提交`d47bd13`加入episode-held-out structured Event-Option离线probe。seed7中连续index ridge selector在disturbed normalized 6D MSE上得到`0.14407`，明显差于train-global continuous index的`0.08075`和fixed4的`0.08847`；bilinear candidate-cost ridge只读预试为`0.15794`，matched no-current为`0.11878`，均正式no-go。提交`3429f69`至`63169ae`进一步实现222874参数的10-way cached-token selector，无observation-to-action residual，夹爪固定来自token4；把错误的soft-mixture MSE改为expected candidate risk后，hard selector在disturbed上从`0.08878`改善至`0.08116`，但仍未优于train-global continuous的`0.08111`，matched no-current为`0.08135`，当前观测贡献仅约`0.23%`。soft mixture诊断为`0.07137`、no-current为`0.07154`，说明收益主要来自不可部署的plan-conditioned凸混合而非最新观测；故精确phase/token selector整体停止，不运行闭环。

提交`0405e8a`完成`1:8 + age4 Event-Failure refresh` router probe，主协议只输入controller intended prefix `cached_actions_env[:4]`，明确排除实际注入的scale/drop/gripper fault action以避免branch泄漏；current与matched no-current各独立训练500步。oracle风险高度集中：25% refresh budget可移除disturbed cached-token4风险的`70.56%`；以full ACoT `95.844 ms`和router `2 ms` guard估算，保守fixed-calendar速度为`6.29x`，refresh后重置计划的renewal估算为`6.86x`。但正式test的total-risk AUROC仅为current `0.58884`、no-current `0.58563`；positive root-nominal excess AUROC为`0.49935/0.49560`。current在25% budget只关闭`48.33%` oracle total-risk gap，低于50%门槛且差于no-current的`50.00%`；四项hard gate中仅速度通过。因此Event-Failure router也判no-go，不调阈值、不运行LIBERO闭环。所有Event-Option、token selector与refresh-router阶段结果均已用现有飞书脚本成功通知，最后一次明确说明服务器可关闭；数据盘在实验前仍约剩137 GiB，本轮新增checkpoint与summary规模很小，无扩容需求。

2026-07-31（Asia/Shanghai）完成branched continuous action corrector的两轮seed7快速验证，代码提交`8053e2e`至`7e76603`并在`ssh -p 30803 root@connect.westb.seetacloud.com`运行。第一轮37.28万参数direct模型读取anchor/current双视角64×64图像、state、cached EAR/IAR、cached action plan和controller-intended前4步，输出cached action 4:8上的连续4步residual；current与独立no-current各训练700步，均由episode-held-out validation选择step1。test 6D action MSE为current `0.0035402542`、no-current `0.0035401196`、stale cached chunk `0.0035426584`、train-only privileged branch residual `0.0035194505`，current相对stale只降低`0.0679%`且略差于no-current，质量门槛失败。GPU sidecar mean/p95为`0.2762/0.3302 ms`，按固定1:4和full ACoT `95.844 ms`估算理论`3.9657x`，只有速度门槛通过。

第二轮保持相同forward输入并改为完整same-root六分支成组采样，在absolute action loss上增加disturbed-minus-nominal paired counterfactual loss，checkpoint按validation disturbed absolute risk与paired risk的等权归一化分数选择。current在test把paired action-delta MSE从zero-pair `0.0014960670`降至`0.0012875309`，关闭`13.94%`分支差异gap，说明成对监督产生了有限信号；但总体6D MSE恶化至`0.0039906432`，比stale高`12.65%`，也比独立no-current `0.0035332129`差`12.95%`，低于预设25% paired closure且没有current增益，故仍判no-go，不启动普通plan-only训练。p95为`0.3173 ms`、理论`3.9650x`，空间仍剩约137 GiB。结果位于服务器`/root/autodl-tmp/acotvla/branched_action_corrector/{20260731_direct_seed7_8053e2e,20260731_paired_direct_seed7_7e76603}`；最后结论已通过现有`scripts/watch_experiment_feishu.py --test_message`成功发送并输出`feishu_notification_verified`。

直接计时审计同时否定了“fresh observation + cached EAR/IAR + full final expert”作为主加速方案：历史150次同checkpoint测量中full policy为`77.2211 ms`，分阶段为prefix/VLM `31.2395`、IAR `1.0548`、EAR `18.1329`、final `19.2591`和profile overhead `7.5347 ms`；已有cached-EAR override为`59.2252 ms`，IAR仅占约`1.18 ms`，所以即使补IAR override也预计仍约`58 ms`、仅约`1.33x`。保留最新视觉时prefix加final已约`50.3 ms`，跨时刻复用完整prefix KV又会使当前LIBERO checkpoint失去最新视觉条件，因此该路径只适合作为系统baseline，不再实施额外IAR训练。

当前数据可辨识性诊断表明seed7 split只有78/80/80 roots和468/480/480 branches；CPU ridge/RBF在首动作上最多只关闭约`3.43%`stale risk，current相对no-current额外约`1.61`个百分点且task-cluster区间跨0，overlap-6 chunk中current还差于no-current。现有direct也只是小型全局chunk head，不是论文级A2C2复现；为避免把容量不足误判为方法失败，已提交`35d5828`加入显式读取每个aligned cached action、sin/cos chunk position和四层共享self-attention的per-token H6 head。与此同时在port8042启动50999 policy server，并开始采集新的10 tasks×10 episodes×最多8 roots canonical same-seed branched数据至`/root/autodl-tmp/acotvla/action_cot_branches/20260731_canonical800_ep10_7e76603`；采集使用两次snapshot determinism gate并保持六分支/固定age4协议，当前仍在运行，尚无最终root数或训练结果。

2026-07-31（Asia/Shanghai）完成expanded canonical branched数据采集。数据集`/root/autodl-tmp/acotvla/action_cot_branches/20260731_canonical800_ep10_7e76603`覆盖LIBERO-10的10个任务、每任务10个episode、每episode最多8个root，共新增800个root；nominal、packet fault、action scale down/up、translation pulse和gripper fault六个分支各800条，总计4800条有效branch，失败root为0。两次快照重放的physics、EEF和pixel最大误差均为0；采集耗时`1427.844 s`，summary明确只支持open-loop same-root标签结论，不能声称closed-loop成功率。采集结束后已关闭collector和port8042 policy server释放GPU；数据盘在采集前仍约剩137 GiB，本数据规模远低于扩容阈值。

随后按用户要求跳过代码复核、单元测试和smoke，服务器直接fast-forward到提交`35d5828`并正式训练121.19万参数的A2C2-Lite式`per_token` direct corrector。模型在严格age4后的H6合法cached-action窗口上读取每个aligned base action、sin/cos位置和四层token self-attention，保持same-root paired action loss；三档current-only训练均为2000 steps。`1e-5/3e-5/1e-4`的test 6D action MSE分别为`0.0025826560/0.0025456678/0.0024552862`，相对stale `0.0026207776`分别改善约`1.45%/2.87%/6.32%`；paired gap closure分别为`0.21%/1.17%/7.24%`，因此`1e-4`最优但仍低于预设10% stale improvement和25% paired closure门槛。三档p95约`0.509–0.541 ms`、理论固定1:4加速约`3.94x`，速度门槛全部通过。

对最佳`1e-4`配置重新独立训练matched current/no-current后，current test 6D MSE为`0.0024460205`，no-current为`0.0025159593`，stale为`0.0026207776`；current相对stale改善约`6.67%`，相对no-current改善`2.7798%`，paired action-delta MSE从zero-pair `0.0009212090`降至`0.0008490265`、关闭`7.8356%`gap，gripper sign accuracy为`99.184%`。p95为`0.5409 ms`、理论`3.9421x`。最新观测存在小幅可利用信号，但current增益略低于3%且paired closure远低于25%，正式offline gate失败；因此该A2C2-Lite版本判no-go，不启动普通plan训练。结果位于服务器`/root/autodl-tmp/acotvla/branched_action_corrector/20260731_a2c2lite_h6_{lr1e5,lr3e5,lr1e4,lr1e4_matched}_35d5828`；本轮结论已通过现有飞书脚本发送，命令退出码0且未输出凭据。下一轮应显式分离root/common prior与observation-conditioned branch correction，而不是继续只调H、K或学习率。

2026-07-31（Asia/Shanghai）完成Counterfactual Dual-Subspace EAR Writeback系列快速迭代，提交`5f62082`至`661575a`并全部在expanded 800-root、episode-held-out split7、严格age4/H6协议上正式训练；按用户要求没有运行代码复核、单元测试或额外smoke，仅在阻断运行时切换执行环境。核心方法用train-only rank-4 nominal absolute EAR residual basis作为cache-only prior子空间，用rank-4 same-root disturbed-minus-nominal EAR basis作为observation feedback子空间，action只能由revised EAR经plan-only decoder产生；同一checkpoint以`current=anchor`得到prior-only因果对照，禁止branch id、实际注入fault action及observation-to-action旁路。

首个2000-step dual-subspace版本参数量41.47万，test full相对prior-only改善`7.67%`，paired action closure从A2C2-Lite的`7.84%`提升到`15.92%`，paired EAR closure`10.38%`，总体action MSE为`0.0024637750`、相对stale改善约`5.99%`，p95 `0.4770 ms`、理论`3.9569x`；达到快速继续线但未过10% overall、25% action-pair和25% EAR-pair论文门槛。延长到6000 steps后paired action closure达到`25.59%`、full相对prior-only达到`17.46%`，但总体MSE恶化到`0.0026393493`、比stale高约`0.71%`，表明feedback支路吸收了common progression。

提交`ff289cc`把监督显式分解为prior拟合nominal fresh EAR/action、feedback拟合branch-minus-nominal并约束nominal feedback为0。4000-step版本总体MSE`0.0025181179`，相对stale改善`3.92%`且优于train-only strong baseline `0.0025543573`；full相对prior-only改善`5.28%`，paired action/EAR closure为`17.26%/10.74%`，p95 `0.3689 ms`。8000-step版本paired action提升到`22.43%`，但总体改善退至约`1.31%`且validation最佳提前到step5000，故停止增加训练量。

提交`e11019f`加入独立current-minus-anchor image/state innovation encoder并用stopped-gradient prior context调制；4000-step结果总体只比stale改善约`0.62%`、paired action/EAR closure为`13.15%/7.90%`，不如shared-delta。提交`c701add`加入冻结ImageNet ResNet-18离线特征缓存；服务器torchvision权重44.7 MiB下载成功，但现有PyTorch 2.7 CUDA wheel不支持Blackwell sm_120，GPU提取阻断后改用32线程CPU，约38秒完成11200张双视角图像的512维特征，缓存`/root/autodl-tmp/acotvla/action_cot_branches/20260731_canonical800_ep10_resnet18_features.npz`仅9.8 MiB。冻结特征版本总体改善`1.65%`、current相对prior-only`2.04%`、paired action/EAR closure`12.15%/5.51%`，validation最佳step1100，正式no-go。

提交`3c7fb5c`把全局flatten EAR decoder改为plan-only token decoder：H6 action query与完整15个revised EAR token通过四层联合self-attention解码，仍无observation旁路。4000-step结果paired action closure`23.60%`，但总体MSE`0.0028508131`、比stale高`8.78%`，validation最佳step2100。提交`661575a`进一步先用fresh EAR→fresh H6 action做2000-step teacher-forced compiler预训练，再以compiler consistency loss训练4000-step writeback；compiler预训练耗时`37.64 s`、主训练`110.01 s`，最终paired action closure`23.67%`、full相对prior-only`5.36%`，但总体MSE`0.0029370310`、比stale高约`12.06%`，paired EAR closure仅`8.11%`。p95仍为`0.5101 ms`、理论`3.9384x`，质量gate失败。

全部迭代均显示速度不是瓶颈，且latest observation在same-root pair上存在真实信号；但轻量EAR writeback无法同时保持总体action quality与达到paired EAR/action门槛。最强paired版本牺牲总体质量，最平衡版本只有`3.92%` overall改善与`17.26%` paired action closure；冻结通用视觉特征和teacher-forced compiler均未解决冲突。因此停止继续搜索训练步数、decoder、随机初始化视觉head或普通plan updater，下一路线转向原ACoT模型内部的partial recomputation/selective reasoning-state reuse，在保留最新视觉与CoT语义的前提下直接复用昂贵中间计算。每轮结论均通过现有飞书脚本发送，命令退出码0且未输出凭据；新增数据、checkpoint和9.8 MiB特征缓存远低于现有约137 GiB空间余量，无需扩容。

2026-07-31（Asia/Shanghai）按Task8快速实际成功率验证需求新增独立闭环runner `scripts/eval_libero_branched_corrector.py`，未修改原`eval_libero_action_cot_pruning.py`。新runner支持同初始state配对的`full_h4`、`stale_h10`和`corrector_h10`三臂：corrector臂在fresh 10/10 ACoT anchor调用中导出并缓存normalized state/EAR、IAR及10步环境动作，先执行前4步，再于最新观测上用本地norm state和saved direct H6 checkpoint替换后6步，全程不在中点增加policy RPC。脚本支持`task_start/max_tasks`、`trial_start/num_trials`、逐局JSONL追加续跑、逐局summary更新、paired rescue/regression统计，以及steps/calls/timing、action delta/jerk、gripper flip、饱和率和corrector边界跳变等诊断；默认Task8且`trial_start=10`避开corrector collector使用的episode 0-9。按用户要求本轮只完成实现，没有运行代码复核、单元测试、smoke或真实LIBERO闭环，因此当前没有新增success结果。

随后在`ssh -p 30803 root@connect.westb.seetacloud.com`完成Task8 `put both moka pots on the stove`的首次真实闭环快速门，代码提交`792a69e`。协议使用corrector训练未见的initial-state IDs 10–19、seed7、10/10 Action-CoT denoising，并对相同10个初始状态依次运行三臂。`full_h4`成功`6/10`、平均控制步数`651.2`、平均policy calls `163.2`；`stale_h10`成功`9/10`、平均控制步数`508.0`、平均policy calls `51.2`；`corrector_h10`成功`7/10`、平均控制步数`683.6`、平均policy calls `68.7`。corrector相对stale为`0` rescue、`2` regressions、`7` both-success和`1` both-failure，净成功率差`-20`个百分点；相对full-H4为`2` rescues、`1` regression，但仍不及stale-H10。由于路径变长，corrector虽然除首次JIT外单次稳态约`1.09–1.36 ms`，平均episode的policy调用与累计wall都高于stale；该版本未形成Task8 success-speed Pareto收益，按预设“有regression且无rescue”快速停止线不扩到25局。结果位于`/root/autodl-tmp/acotvla/branched_action_corrector/20260731_task8_closedloop_ep10_19/{episodes.jsonl,summary.json}`，飞书通知命令退出码0并已输出`feishu_notification_verified`。由于样本仅10个，当前证据只支持否定该checkpoint的快速门，不能宣称整个midpoint-correction方法在统计上不可能；下一步若继续诊断，应在同一age4 MuJoCo snapshot上比较stale/corrector/fresh-teacher H6造成的两个moka-pot真实目标进度与teacher handoff恢复，而不是继续把总体action MSE当最终判据。

用户随后要求后续同类闭环比较每个方法运行20次。三臂含义已统一：`full_h4`为每次完整ACoT推理只执行4步后重新规划的高频fresh参考，并非理论质量上界；`stale_h10`为一次完整ACoT后直接执行全部10步，其中第5–10步不读取新观测；`corrector_h10`为先执行原计划4步，再由轻量current-observation corrector替换后6步，10步后才重新完整ACoT。后续协议应使用20个paired Task8 initial states；若扩展当前结果，可复用10–19并新增20–29，合并成每臂20局，避免重复计算。当前10局中corrector对stale的exact paired discordance只有2个且均为regression，因此point direction为负但样本不足以作统计性方法总否定；更准确结论是“当前无条件每4步必改的direct checkpoint不理想”，而非所有midpoint correction都无效。

2026-07-31（Asia/Shanghai）开始实现无条件direct corrector之后的选择性纠正诊断，新增独立离线脚本`scripts/probe_branched_corrector_confidence.py`，不修改原LIBERO eval。脚本复用corrector trainer的task-stratified、episode-disjoint split和saved direct current/no-current checkpoints；同一个current checkpoint分别读取真实current observation与`current=anchor`，由两次输出的observation response、完整correction norm、response/correction ratio、state/image delta、action boundary jump、delta、jerk和gripper disagreement等部署可得特征训练小型ridge benefit predictor，标签为每样本stale H6 6D MSE减current-corrector H6 6D MSE。ridge强度和带Wilson回归率上界约束的保守routing threshold只在validation选择，test将报告AUROC、budgeted gain、coverage、selected current-vs-stale MSE、regression rate及nominal retention。脚本还加入两个以stale为锚且保留stale夹爪的更保守候选：same-model counterfactual response `stale + alpha * (f_current(real)-f_current(anchor))`，以及matched difference `stale + alpha * (f_current(real)-f_no_current(anchor))`；alpha和逐token连续6D L2 trust region只在validation扫描，冻结后test报告overall/nominal/disturbed MSE和paired action-gap closure。本轮仅完成代码实现，尚未在服务器运行，因此没有新增离线指标或Task8成功率结果；按用户要求没有运行代码复核、单元测试或smoke。

2026-07-31（Asia/Shanghai）将validation-only probe选出的保守matched-difference候选接入真实评测代码，尚未提交或运行。`scripts/eval_libero_branched_corrector.py`新增`matched_diff_h10`并保持原三种mode兼容：age4时分别运行current checkpoint的真实current输入和独立no-current checkpoint的anchor替代输入，以stale H6为锚，仅叠加`alpha=1.0`的两路输出差，逐token连续6维L2限制为`0.025`，夹爪完全保留stale；逐局记录双路及总corrector耗时、response norm、applied norm和clip fraction，默认trial数为20。`scripts/audit_task8_corrector_short_cf.py`新增`--candidate matched_difference`复用同一公式，并新增`--audit-root-indices 10 25`以只记录代表性后期macro roots、同时沿stale轨迹推进经过更早roots。本轮按用户要求没有运行代码复核、单元测试、smoke或训练/闭环，因此没有新增动作质量或成功率指标。

2026-07-31（Asia/Shanghai）为继续定位Task8 direct corrector失效原因，新增独立脚本`scripts/audit_task8_corrector_short_cf.py`，未修改既有闭环或普通eval。协议默认只运行corrector训练未见的Task8 initial-state IDs 10–29共20次，并沿实际`stale_h10`轨迹采集可选数量的中间root：每次fresh ACoT先执行H4，在age4捕获`collect_action_cot_state_branches.py`的canonical MuJoCo/controller snapshot，再从完全相同状态比较cached stale H6、saved direct corrector H6与fresh 10/10 teacher H6；stale与corrector端点分别接使用同一policy seed的fresh teacher H9，以区分H6局部动作质量与handoff后的可恢复性。评分不使用总体action MSE，而从`privileged_progress.py`严格提取Task8两个`On(moka_pot, stove_cook_region)`谓词，记录两pot continuous-progress sum、满足数、H6及H6+H9的paired wins/losses/ties，同时记录三臂action RMSE、gripper分歧和restore起点spread。脚本支持`--trial-start`、默认`--num-trials 20`、`--max-roots-per-episode`、`roots.jsonl`/`episodes.jsonl`即时追加续跑及`summary.json`滚动汇总；按效率要求本轮未运行代码复核、单元测试、smoke或服务器实验，因此尚无新数值结论。

2026-07-31（Asia/Shanghai）完成Task8 matched-difference corrector的same-snapshot短反事实与三轮20次闭环止损验证，相关实现提交为`aa422cd`、`58b07d7`、`a4abc90`、`6bc3ea9`和`35194fd`。IDs 30–49的40个短反事实root中，matched-difference相对stale在H6端点为23胜/5负/12平，接同seed fresh-teacher H9后为20胜/12负/8平；其中root10在handoff后9胜/11负，root25为11胜/1负/8平。持续late-gate在IDs 10–29上与stale同为16/20，1 rescue/1 regression；只在control step 250–259尝试一次且response L2不超过0.025的版本一轮为17/20、1 rescue/0 regression，但消除非触发时无用cache导出与观测预处理后确认轮回到16/20、0 rescue/0 regression，唯一rescue未复现。优化版稳态corrector约7.68 ms/episode，但steps/calls为557.55/56.1，高于stale的546.75/55.0，稳态端到端约慢3.9%。因此无条件、持续门控和单次门控三种learned corrector均未形成稳定Pareto增益，当前corrector路线停止；各轮飞书通知均返回`feishu_notification_verified`。

2026-07-31（Asia/Shanghai）提交`98f8754`新增`event_refresh_h10`上界baseline：通常仍执行完整H10，仅在control step 100–109覆盖的单一macro-cycle先执行anchor H4，再从最新观测额外调用一次完整10/10 ACoT并执行fresh H6；该模式不是学习到的trigger，固定位置来自独立校准IDs 30–49上root10 fresh-teacher相对stale H6的13胜/7负与正平均进度。正式IDs 10–29、seed7为stale 16/20、event-refresh 19/20，3 rescue/0 regression，steps 477.0 vs 546.75、calls 49.25 vs 55.0、同轮policy wall 5.329 s vs 6.022 s、端到端13.130 s vs14.594 s；额外stale复现仍为16/20且逐episode完全一致。第二个policy seed107为stale16/20、event-refresh18/20，3 rescue/1 regression，steps447.9 vs534.0、calls46.25 vs53.75、policy wall4.155 s vs4.808 s。

两个seed合并40对为stale32/40（80.0%）、event-refresh37/40（92.5%），6 rescue/1 regression、净+5；平均steps从540.375降至462.45（-14.42%），calls从54.375降至47.75（-12.18%），同轮policy wall从5.415 s降至4.742 s（-12.44%），端到端从14.131 s降至12.464 s（-11.80%）。7个discordant pair的exact McNemar双侧p=0.125、单侧p=0.0625，因此当前只支持把“稀疏事件点fresh re-reasoning”作为跨seed一致的promising pilot，不支持统计显著或论文最终结论。下一步应保留exact refresh作为上界，把固定control step替换为部署可见的Action-CoT语义事件/plan-validity trigger，并在更多task/root/seed上验证后再蒸馏partial refresh；seed107与合并结论已通过飞书发送且返回`feishu_notification_verified`。

2026-07-31（Asia/Shanghai）继续把固定event-refresh改成两种无需全局step的部署规则，提交`1673f88`与`7e1c08c`，按用户要求直接运行20次而未增加代码复核、单元测试或smoke。`gripper_event_refresh_h10`以最初5个有效夹爪指令多数票推断open sign，在实际指令连续2步翻转为close后丢弃剩余stale chunk并exact refresh一次；Task8 seed7为16/20，相对stale 1 rescue/1 regression，触发step均值114.8、中位111.5、范围90–179，steps/calls为572.6/58.45，判no-go。`predicted_gripper_event_refresh_h10`在当前10步chunk预测到连续2步close时提前执行H4后exact refresh；结果15/20、1 rescue/2 regressions，触发step均值112、中位114、范围84–174，steps/calls为588.05/60.1，同样no-go。两次结论均通过飞书发送并返回`feishu_notification_verified`。

本轮最终证据边界是：固定control step 100–109的exact fresh re-reasoning在两个policy seed上均净正并形成Task8 success/calls pilot Pareto，但两种只依赖gripper事件的手工部署trigger都不能复现，说明收益来自更细的plan-validity/位姿状态，而不只是抓取开合事件。下一实现应训练Action-CoT plan-validity/event-benefit head：输入单次正常调用已有的prefix、EAR/IAR、final chunk、state及plan-history，标签来自同snapshot stale-H10与H4+fresh-H6的paired terminal success、progress和remaining calls；先用不对称regression-aware gate选择是否执行一次exact refresh，之后才蒸馏partial refresh。当前port8042模型服务与评测进程均已停止，GPU无残留compute进程，数据盘此前仍约136 GiB可用。本地GitHub push从提交`98f8754`起按用户要求显式使用`127.0.0.1:7897`代理。

2026-07-31（Asia/Shanghai）完成Task8 exact plan-refresh benefit数据与两版deployable temporal gate验证，相关提交为`f844366`、`89e3f72`和`abf8d8e`。collector在episode IDs 0–9、30–49的root 6–14采集270个same-snapshot paired roots，比较cached stale H6与最新观测上的exact fresh H6并接同seed fresh H9；H6 fresh-minus-stale normalized progress均值为`+0.0010748251`，H6+H9均值为`+0.0000883281`，handoff为130胜/117负/23平，证明无条件refresh噪声较大。v1 ridge只读取正常ACoT调用已有的256维temporal feature，在held-out episode 40–49验证集选择3/10、3胜0负，但feature与benefit相关性为`-0.19375`。正式两seed合并为stale `32/40`、plan-refresh `33/40`，1 rescue/0 regression，policy calls均值均为53.75、policy wall约降低1.5%，只得到很弱的净正结果，明显低于固定event upper-bound的`37/40`。

修正trainer与线上“首次越阈值即触发”不一致后，v2阈值在validation选择8/10、6胜1负；正式复用seed7同批20个stale基线并只跑learned arm，最终仍为`16/20`，1 rescue（episode15）和1 regression（episode23），触发10/20。相对stale，policy calls `55.6→56.5`（`+1.6%`）、policy wall `6.035→6.228 s`（`+3.2%`）、端到端`14.611→14.794 s`（`+1.3%`），故v2明确no-go且不追加第二seed。首次启动该轮时漏传FFmpeg `LD_LIBRARY_PATH`导致`libavformat.so.61`导入失败；确认库仍在`/root/autodl-tmp/acotvla/ffmpeg-7.1.1/lib`后用完整环境重启，失败启动没有占用GPU或写入实验数据，最终20局结果完整。结果已通过飞书脚本发送并得到`feishu_notification_verified`；数据盘仍约136 GiB可用，无需扩容。当前结论是旧anchor temporal feature缺少执行H4后的真实偏差信息，下一轮应改用latest-observation与cached EAR/IAR/action plan的self-supervised plan-consistency innovation gate，而不是继续调threshold、H或K。

2026-07-31（Asia/Shanghai）提交`1a92e86`并直接完成spatial plan-validity v0离线probe，未运行单元测试、代码复核或额外smoke。脚本`scripts/probe_plan_validity_spatial.py`在现有270个Task8 same-snapshot refresh roots上提取冻结ResNet18 layer3的4×4双视角空间特征；所有channel PCA、expected state-delta ridge、归一化和benefit ridge均限制在episode-held-out fold内，特征只含anchor/current observation、state和cached EAR/final/env plan，禁止fresh信息、root index及绝对step。主模型与`current=anchor`和同root跨episode打乱current两个matched control比较，train为episodes 0–9及30–39、development held-out为40–49。

v0结果明确no-go：real-current OOF Spearman为`-0.02843`，anchor为`-0.09418`、shuffle为`-0.11615`，真实current相对最好control仅提高`0.06575`，低于预设`0.10`且5 folds仅1 fold选择收益为正。held-out Spearman为`0.04386`，保守first-crossing只触发1/10，mean benefit `+0.00006441`、1 win/0 loss、oracle-gap closure仅`0.2418%`；fixed phase root10 held-out mean为`-0.00029134`。因此空间位置、raw visual delta及由Task8 nominal roots拟合的expected-state innovation仍不能稳定区分有益与有害refresh；现有270条nominal deterministic roots的phase与current高度绑定，继续堆CNN或阈值不合理。下一轮转向同anchor、同root下多种H4执行结果的paired disturbed-current数据，让模型从受控current变化中学习`真实反馈 × 未执行Action-CoT tail`的refresh utility。结果位于服务器`/root/autodl-tmp/acotvla/plan_refresh_benefit/20260731_task8_spatial_validity_v0_1a92e86/summary.json`，已通过飞书发送并得到`feishu_notification_verified`；运行只耗约6秒且未新增显著存储。

2026-07-31（Asia/Shanghai）为解除270条nominal plan-refresh roots中phase与current observation的绑定，新增独立collector `scripts/collect_libero_plan_validity_branches.py`，未修改原collector或LIBERO eval。默认协议为Task8 episodes 0–9和30–49、macro roots 8–12、seed7；每个root先从同一个pre-H4 canonical snapshot分别执行`nominal/underact/overact/translation_pulse/gripper_fault`五种由`collect_action_cot_state_branches.make_branch_actions`生成的H4 prefix，再在各branch endpoint从同一snapshot比较cached stale H6与exact fresh H6，并为两臂请求同一continuation seed的fresh H9。每个branch独立原子NPZ作为resume authority，保存anchor/current双视角图像与normalized state、cached/fresh EAR/IAR/final/env计划、intended/requested/executed prefix、branch元数据、H6和handoff progress advantage及predicate guard；prefix提前终止会写`valid=false`样本。所有branch采集后环境恢复pre-H4 snapshot，episode主轨迹只执行nominal cached H10。当前只完成实现，尚未在服务器启动采集，因此没有root数、耗时、指标或存储增量；按用户要求没有运行代码复核、单元测试或smoke。

2026-08-01（Asia/Shanghai）完成Task8 multi-current paired plan-validity正式采集。数据目录`/root/autodl-tmp/acotvla/plan_validity_branches/20260731_task8_ep0_9_30_49_roots8_12_seed7_1362eb5`包含episodes 0–9及30–49、roots 8–12、五种H4 branch共750个独立NPZ，30/30 episodes完成，0 failed、0 invalid；episode采集耗时合计779.21秒，数据92 MiB，数据盘仍剩136 GiB。handoff fresh-minus-stale平均benefit按branch为gripper fault `+0.01291`、nominal `+0.00247`、underact `-0.00337`、overact `-0.00699`、translation pulse `-0.00340`，说明“检测到扰动就refresh”不成立。privileged branch-name、root及root×branch的5-fold episode-OOF排序也只有Spearman约`-0.0007/-0.0658/+0.0343`，within-root pairwise最多`52.87%`，表明单次H6+H9 progress advantage跨episode不稳定。

提交`cea82ba`和`a5b1ff4`新增离线`scripts/train_verified_acot_gate.py`并明确partial probe不能作go判断。probe以冻结ResNet18 layer3空间特征、train-fold PCA、nominal-only expected visual/state transition和same-root pairwise benefit regression训练；primary只读anchor/current observation/state、policy-issued intended prefix及cached EAR/IAR/final/env plan，排除branch/root/step、actual injected prefix、fresh信息和privileged outcome，actual-prefix telemetry只作为单独oracle。正式750样本、30-episode、5-fold handoff OOF中，real overall/disturbed Spearman为`-0.09867/-0.11041`、within-root pairwise `50.20%`，不优于current=anchor、shuffled-current、tail-shuffle，telemetry oracle也仅`50.80%`，故`go=false`且未写deployable checkpoint。改用更局部H6 label后real Spearman为`+0.00264`、pairwise `50.03%`，shuffled-current反而`+0.00927`；143/150 off-policy trajectories几乎全触发，正mean不能解释为选择能力，同样`go=false`。两轮结果均已通过飞书发送并得到`feishu_notification_verified`。当前证据否定继续堆spatial encoder或linear head；下一轮改采same-snapshot stale-vs-exact-refresh两臂到task terminal的真实success/value-of-replanning标签。

2026-08-01（Asia/Shanghai）新增独立terminal-label collector `scripts/collect_libero_terminal_refresh_pairs.py`，未修改原collector或LIBERO eval。默认只读取Task8 development episodes 0–9及30–49，参数层显式拒绝formal episodes 10–29，并固定macro root10与nominal、underact、overact、translation pulse、gripper fault五种H4 branch。每个branch从同一个post-H4 canonical snapshot分别执行cached stale H6与endpoint exact-ACoT fresh H6，随后两臂共享原episode absolute step limit，按预注册absolute decision slots `anchor_step+10+10*j`调用`canonical_policy_seed`并正常full-H10 replanning至success或step limit；fresh endpoint请求仍使用anchor root seed。每branch独立原子NPZ保存terminal success差、terminal privileged progress差、H6诊断、remaining steps/calls/wall、anchor/current observation及cached/fresh EAR/IAR/final/env计划。成本字段明确拆分common sunk anchor、fresh decision refresh RPC和各臂decision-to-terminal成本，另存physical two-arm collector RPC但标注不能作为部署成本。当前仅完成代码，尚未在服务器启动，所以没有pair数、terminal结果、实际耗时或存储增量；按用户要求没有运行代码复核、单元测试或smoke。

2026-08-01（Asia/Shanghai）在服务器`ssh -p 30803 root@connect.westb.seetacloud.com`启动Task8 terminal refresh-pair采集，输出目录为`/root/autodl-tmp/acotvla/terminal_refresh_pairs/20260801_task8_ep0_9_30_49_root10_seed7_167b340`。前5个完整episode得到25/25 valid pairs：stale与always-fresh均为20/25 success，配对为3 rescues、3 regressions、19 ties；rescue覆盖3个episode/3种branch，regression覆盖3个episode/2种branch，per-pair选择oracle为23/25，因而仅支持继续采满以测试选择门控，不支持无条件fresh。17个双成功pair中，fresh含extra refresh RPC后的decision-to-terminal calls为8个更少、2个相等、7个更多，均值差`-4.18`但中位数0。数据字段检查确认current observation/state、cached EAR/IAR/action tail、fresh counterfactual及terminal标签完整。另修复invalid prefix样本已写盘时索引摘要无条件读取缺失H6字段可能导致resume中断的问题：`_index_row`现在对该诊断字段使用NaN默认值；正在运行的样本截至记录时尚未出现invalid，采集未重启且仍在继续。数据盘仍剩约136 GiB，无需扩容。

2026-08-01（Asia/Shanghai）完成Task8 terminal refresh-pair采集及一次冻结协议的终局门控评估，代码最终HEAD为`d4a97bb`。collector在episodes 0–9、30–49的固定root10和五种branch上得到150/150 valid pairs、30/30 episodes完成、0 invalid、0 failed；总体stale为129/150、always-fresh为122/150，14 rescues、21 regressions，per-pair oracle为143/150，证明无条件fresh有害但存在选择性headroom。新增`scripts/train_terminal_refresh_gate.py`使用train episodes 0–9及30–39的episode-grouped 5-fold OOF冻结模型与阈值，并只打开一次dev 40–49；primary只读取决策前可用的current image、cached EAR/IAR/action tail与intended prefix，排除branch/root/step、actual injected prefix、fresh Action-CoT输出、privileged outcome以及fresh RPC返回的`current_state`。train OOF阈值选择14/100，3 rescues、2 regressions、净成功`+1`，policy calls相对stale少130，但违反零regression episode风险约束；real OOF Spearman仅`0.0335`且不优于anchor control。held-out dev的stale为44/50、always-fresh为39/50，5 rescues、10 regressions、oracle为49/50；冻结real gate因score分布漂移选择0/50，得到0 rescue/0 regression、成功44/50，Spearman `-0.16146`、episode-equal pairwise `37.5%`，低于anchor-current与shuffled-current controls，因此`go=false`且未保存deployable checkpoint。数据位于`/root/autodl-tmp/acotvla/terminal_refresh_pairs/20260801_task8_ep0_9_30_49_root10_seed7_167b340`（20 MiB）和`/root/autodl-tmp/acotvla/terminal_refresh_gate/20260801_task8_terminal_lex_5fold_d4a97bb`（148 KiB）；数据盘仍剩136 GiB。结果已通过飞书发送并输出`feishu_notification_verified`。当前结论是terminal success/value-of-replanning标签确认了约10个百分点的dev选择上限，但现有冻结视觉特征加linear ridge无法泛化；不进入闭环，也不应继续在同一表示上调阈值。collector已退出，policy server仍在tmux `plan_validity_server`等待用户关机。

2026-08-05（Asia/Shanghai）在恢复的westb服务器上完成post-query paired Action-CoT chooser快速go/no-go，代码提交为`bb38748`，新增独立脚本`scripts/train_terminal_paired_cot_chooser.py`；按效率要求未运行代码复核、单元测试或额外smoke，而是直接在现有150个Task8 terminal pairs上训练。方法在一次exact fresh ACoT已经完成后，以共享的14,401参数candidate scorer对cached候选（EAR phase 2、action 4:10）与fresh候选（EAR phase 0、action 0:6）评分，使用episode-grouped 5-fold OOF、regression-weighted terminal preference loss，并把fresh query作为两种commit选择共同承担的sunk cost；输入排除了branch/root/step、实际扰动prefix和terminal outcome。train OOF中real chooser选择15/100，得到2 rescues、1 regression和86/100 terminal successes，弱于stale 85、尤其弱于无需学习的branch-only 89；fresh-shuffle控制同为2 rescues、1 regression和86/100。已被前轮使用过、仅作诊断的episodes 40–49上，real选择4/50，得到0 rescue、1 regression和43/50 successes，低于stale 44及branch-only 45；所有预注册go checks中只有real净收益优于current-anchor成立，`go=false`。结果位于`/root/autodl-tmp/acotvla/terminal_paired_cot_chooser/20260805_task8_postquery_shared_v0_bb38748`，日志为同级`train_v0_bb38748.log`；服务器数据盘约剩136 GiB，无需扩容。该结果支持停止pre-query gate与post-query chooser整个snapshot-sidecar路线，不增加模型容量、不补采同类数据且不进入闭环；当前唯一稳定正结果仍是固定稀疏exact-refresh在两个seed上将Task8从32/40提高到37/40并同时减少约12%的calls/wall，但固定事件尚未泛化为可学习方法。下一步转向直接降低每次CoT推理成本的Fast-ACoT/双流蒸馏闭环验证，而非继续调H、K或门控阈值。本轮结论已通过现有飞书脚本发送，命令返回`feishu_notification_verified`且未输出凭据。

同日随后首次完成B6-lite 1/1 Fast-ACoT的Task8真实闭环检查。服务器直接复用现有完整1.3 GiB endpoint sidecar `/root/autodl-tmp/acotvla/ir_acot_pilot/final_b6/final/params`，该sidecar由2k teacher records、300步顺序EAR/final endpoint蒸馏得到；评测固定LIBERO-10 task id 8、initial-state IDs 10–29、seed7和H=10，仅将每次推理内部的EAR/final flow从10/10降为1/1，因此不是执行H或K的调整。首次eval启动因漏带已有LIBERO `PYTHONPATH`而在导入阶段退出，未进入episode或写出半截结果；恢复`/root/ACoT-VLA/third_party/libero`环境后直接完成正式20局。B6得到15/20，原10/10 ACoT在完全相同IDs、seed与H上的已复现基线为16/20；paired为2 rescues（episodes 14、22）、3 regressions（13、20、24）、13 both-success和2 both-failure，净少1个成功。B6的policy compute从基线83.641降到65.206 ms/call（`-22.04%`），每episode累计policy compute从4.600降到3.700 s（`-19.56%`），RPC wall从4.955降到4.381 s（`-11.59%`）；但平均steps/calls分别从556.75/55.0增至573.95/56.75（约`+3.09%/+3.18%`），完整episode elapsed从13.976增至14.630 s（`+4.68%`）。因此计算路径加速在真实闭环中成立，但当前2k/300-step checkpoint有成功率回退且路径变长，尚未形成成功率—端到端速度Pareto。结果位于`/root/autodl-tmp/acotvla/ir_acot_pilot/closed_loop_b6_task8_ep10_29_h10/eval`；结论已通过飞书发送并返回`feishu_notification_verified`。B6模型服务和eval进程随后均已停止、GPU释放。当前最接近可行的算法路线是增强双阶段少步Action-CoT蒸馏，而不是继续snapshot gate、H/K或threshold搜索；下一轮需提高student闭环保真度后复用同一paired20快速门。

同日继续使用与B6完全相同1/1部署计算图的IR-lite sidecar `/root/autodl-tmp/acotvla/ir_acot_pilot/final_ir/final/params`做真实闭环；IR-lite除clean endpoint loss外，额外约束同观测/IAR/noise下EAR语义干预引起的student最终动作差分匹配10/10 teacher。Task8 IDs 10–29、H10、seed7中IR为17/20、teacher为16/20，2 rescues/1 regression；IR相对同速度B6的15/20救回episodes 13和20且没有新增regression。seed107独立复现为IR 17/20、teacher 16/20，4 rescues/3 regressions。两个seed合并IR为34/40、teacher为32/40，6 rescues/4 regressions；每个seed均净增1，但10个discordant pair不足以支持统计显著。seed7中policy compute/call下降`21.75%`、policy/episode下降`27.30%`、calls/steps下降`7.09%/6.97%`、RPC wall下降`19.77%`、完整elapsed下降`0.79%`；seed107对应下降`22.86%/28.02%/6.70%/6.64%/21.05%/1.38%`。两轮飞书通知均返回`feishu_notification_verified`。

随后完成Task0/6/9各10个paired initial states的跨任务retention哨兵表，固定seed7、H10及相同IDs 10–19。IR在Task0/6/9均为10/10；teacher分别为10/10、9/10、10/10，合计IR 30/30、teacher 29/30，1 rescue（Task6 episode11）、0 regression。三任务聚合policy compute/call从98.487降至60.905 ms（`-38.16%`），policy/episode从2.945降至1.691 s（`-42.57%`），RPC wall下降`39.44%`，calls/steps下降`7.13%/6.76%`，完整episode elapsed从13.508降至11.996 s（`-11.20%`）。分任务中Task0完整elapsed下降`9.13%`、Task6下降`22.23%`；Task9虽然calls/steps增加约`14.13%/13.65%`，policy/episode仍下降`36.94%`、RPC wall下降`35.05%`且完整elapsed仅增加`0.11%`，成功率不退。该结果把Intervention-Response Distilled Fast Action-CoT提升为首个跨policy seed、跨Task0/6/8/9同时出现success-compute Pareto的可行候选，但尚未覆盖全部10个任务或达到统计性论文结论。结果位于`/root/autodl-tmp/acotvla/ir_acot_pilot/{closed_loop_ir_task8_ep10_29_h10,closed_loop_ir_task8_ep10_29_h10_seed107,retention_task0_6_9_h10_seed7}`；飞书通知已验证。当前继续补Task1/2/3/4/5/7各10个paired states，组成10-task×10快速表。服务器剩余空间约136 GiB，无需扩容。

同日完成IR-lite 1/1 Fast Action-CoT的LIBERO-10全任务快速表。协议固定seed7、H10和每任务initial-state IDs 10–19；逐任务teacher→IR成功数为Task0 `10→10`、Task1 `10→10`、Task2 `10→10`、Task3 `10→10`、Task4 `10→10`、Task5 `9→9`、Task6 `9→10`、Task7 `10→10`、Task8 `7→8`、Task9 `10→10`，总计`95/100→97/100`。配对结果为2 rescues（Task6 episode11、Task8 episode14）、0 regressions和3 shared failures；两个discordant pair的exact McNemar双侧p值为0.5，因此当前结果支持跨任务可行性和retention，不支持统计显著的成功率优越性。Task8成功率复用此前相同IDs/seed/H的完整配对结果，其teacher来自旧branched runner，故未把Task8时间混入统一速度统计。

统一runner下的Task0–7及Task9共90对聚合中，10/10 teacher到1/1 IR的policy compute/call为`95.410→60.051 ms`（`-37.06%`），policy compute/episode下降`37.61%`，RPC wall/episode下降`33.97%`，完整episode elapsed为`11.975→11.117 s`（`-7.17%`），calls/steps下降`0.88%/0.93%`。只看88个shared-success pairs时，完整elapsed下降`5.23%`，但calls/steps增加`2.59%/2.45%`；Task3完整elapsed增加`2.92%`、Task9增加`0.11%`，说明总体加速已经成立但仍有少数任务的轨迹长度抵消部分推理收益。补跑结果位于`/root/autodl-tmp/acotvla/ir_acot_pilot/retention_10tasks_ep10_19_h10_seed7/{teacher,ir}_task{1,2,3,4,5,7}`；评测结束后已停止`ir_remaining_server`，未保留本轮GPU服务，空间仍无需扩容。最终10-task结果通过现有飞书脚本和服务器私有环境配置发送，命令退出码0且webhook未报拒绝。

对论文创新边界的本轮审计结论是：clean endpoint 10→1 flow distillation只能作为加速baseline；IR-lite用手工translate/rotate/gripper干预匹配teacher/student的最终动作差分，属于比普通endpoint KD更强的结构化正则，但当前形式仍容易被解释为有限差分Jacobian matching或新的distillation loss，不能单独支撑ICRA主创新。One-Step Flow Policy已经覆盖VLA中的一步flow self-distillation，ECoT-Lite已经覆盖训练时使用reasoning而测试时省略reasoning，通用Jacobian matching也早于本项目。当前IR-lite应定位为可行性pilot和强baseline；若作为主方法，下一版至少需要把手工干预升级为受Action-CoT可行域约束的自动/对抗式counterfactual teacher queries，匹配归一化reasoning-to-control response operator，并让final action loss端到端塑造EAR，同时以干预faithfulness、OOD闭环和matched-compute消融证明其不是普通KD。上述升级尚未实现或验证。

进一步核对DIITO/Causal Distillation后，新颖性边界更严格：teacher/student在对齐中介变量接受interchange intervention后匹配输出变化已有NAACL 2022先例，故IR-lite不能声称首次causal/interventional distillation；其可守住的差异只是在连续、物理可解释的action-space EAR中介上压缩双flow。下一迭代首先应补同一100个initial states上的matched B6 endpoint-only 1/1全任务表，直接隔离IR loss而不是继续只与10/10 teacher比较；现有206条held-out中IR相对B6的response-MSE只改善`0.5701%`且bootstrap区间跨0，clean MSE还恶化`0.1296%`，所以该控制是必要的。

若以ICRA主方法继续，建议把IR-lite降为baseline/辅助损失，核心升级为Causal Coupled Action-CoT Flow：在同一solver round内同时输入coarse和final suffix，让EAR与final action沿一个block-triangular joint flow共同演化，final action读取逐轮更新的EAR，从而改变原先“完整EAR flow结束后再运行完整final flow”的串行生成算法。当前`PaliGemma.llm`已支持三个expert槽位且attention实现可同时接收coarse/action suffix，因此该方向在现有架构上可实现；目前推理代码仍将两分支分开调用。训练辅助项采用multi-time interventional response consistency，在多个flow time/interval匹配EAR干预引起的final velocity/transport变化，并加入held-out EAR interchange审计；distillation仅用于初始化或regularization，不再作为唯一贡献。快速门应先用现有2k数据训练小模型，再跑Task8及Task3/9；只有相对IR-lite显著降低未见干预response error、闭环成功不退且仍保持相对10/10 teacher的实际延迟优势，才扩到全10任务、多policy seed和training seed。该设计尚未实现或运行。

2026-08-05（Asia/Shanghai）完成matched B6 endpoint-only 1/1在LIBERO-10、seed7、H10、initial-state IDs 10–19上的100回合归因表。B6逐Task0–9成功数为`10/10/10/10/10/9/8/10/9/9`，总计`95/100`；同runner IR为`97/100`，IR相对B6有4 rescues（Task6 episodes14/16、Task8 episode17、Task9 episode11）、2 regressions（Task8 episodes10/15）、93 both-success和1 both-failure，净增2，McNemar exact双侧`p=0.6875`。逐任务IR相对B6为2胜7平1负，Task6 `+2`、Task9 `+1`、Task8 `-1`。两者单次policy成本相当（IR/B6 `61.165/60.971 ms`）；但93个共同成功回合中IR相对B6的policy/episode、RPC和完整elapsed分别增加`3.67%/3.95%/5.34%`，calls增加`3.53%`。当前runner可无歧义匹配teacher的Task0–7及9共90回合中，teacher/B6/IR为`88/90、86/90、89/90`；缺失Task8同runner teacher，未拼接其计时。因此IR达到预设净`+2`质量门但统计证据弱且不优于B6的matched-success速度，只保留为质量辅助损失，不能宣称Pareto主方法。该归因结果已通过现有飞书脚本发送并得到`feishu_b6_result_verified`，B6服务随后停止、GPU释放。

同日提交`f3da7f0`实现opt-in Causal Coupled Action-CoT Flow与multi-time IR原型：joint sampler在一个suffix LLM调用中按coarse→final token顺序建立block-triangular attention并同步更新两个flow；trainer默认保持原endpoint路径，启用后在`t=1` endpoint之外加入固定`t=0.5` flow loss及interventional velocity-response loss。提交`9763179`补充对全部episode-heldout validation记录的确定性加权评估和`full_validation_metrics`，便于直接比较B6、IR与MTIR；按效率要求未运行代码复核、单元测试或额外smoke。服务器首先启动final-only MTIR-600快速门，首次进程因继承旧仓库`/root/ACoT-VLA/src`的`PYTHONPATH`而误连Hugging Face并在任何训练step前退出；保留失败日志后固定`/root/ACoT-VLA-multirate/src`、本地12 GiB LeRobot cache和offline模式重启，飞书已发送启动、失败修正及归因结论。MTIR-600在本条记录时仍在加载/运行，尚无训练或验证指标，不能判断有效性。

MTIR-600随后在444.21秒内完成，输出`/root/autodl-tmp/acotvla/ir_acot_mtir_t05_finalonly_s7/final_mtir600`及约1.6 GiB完整sidecar。固定206条episode-heldout全量验证中，B6→MTIR的clean final MSE为`0.0049973773→0.0048163501`（改善`3.62%`），endpoint intervention-response MSE为`0.0018172907→0.0017593268`（改善`3.19%`），response cosine为`0.3763820→0.4422233`；clean和cosine方向为正，但response未达到预注册至少10%改善门，严格判为no-go且未进入闭环。随后从该checkpoint直接加入权重1.0的endpoint IR并保持`t=0.5` flow/response权重`1.0/0.5`短训300步，输出`final_mtir_ir300`；316.32秒后的全量response MSE仅到`0.0017530786`，相对B6总改善`3.53%`、相对上一轮仅额外改善约`0.36%`，clean MSE为`0.0048148224`、cosine为`0.4357124`。该平台期支持停止继续调response loss权重，转向训练/推理同图的Coupled Action-CoT联合flow；两轮完成/no-go结论及下一步均已通过飞书脚本发送并得到`feishu_mtir_result_verified`与`feishu_ir300_result_verified`。服务器数据盘仍约剩136 GiB，无需扩容。

提交`2f99d4a`进一步把Coupled Action-CoT的sampler与trainer统一到同一个`[None,C,A]` block-triangular suffix LLM前向，并把C/A的RoPE位置改为各自从prefix末端开始，避免action因拼接整体后移；新增默认关闭的`--joint-coupled-training`，final-only时冻结C，dual时允许action loss经C的K/V反向塑造coarse hidden。按效率要求实现后未做代码复核、单元测试或smoke，直接训练。final-only joint300在351.19秒后得到全量final MSE `0.0294254`、`t=0.5` response MSE `0.0091340`及cosine `-0.379922`，相对B6 final MSE约为`5.89x`，明显失败；随后只再运行一次允许C/A共同训练的dual300，390.24秒后coarse MSE为`0.000137092`，但final MSE仍为`0.0313553`，`t=0.5` response MSE/cosine为`0.00942956/-0.366456`，触发final `>0.0075`、response `>0.003`、cosine非正三项止损。证据表明same-round action在`t=1`读取noisy C、而监督来自sequential clean-C teacher造成的上下文错配，不能由300步final-only或dual适配修复；dense same-round joint路线正式no-go，未运行速度profile或LIBERO闭环，也不再增加训练步数或调loss。两次结论已通过飞书发送并得到`feishu_joint_result_verified`与`feishu_jointdual_result_verified`。下一方向改为先保留B6 clean、可解释EAR，再将其编译为少量plan latent供轻量action decoder读取，以消除noisy-plan错配并提高B6两次suffix LLM调用之外的速度上限；该新方向在本条记录时尚未实现或验证。

提交`8a384d0`新增独立PyTorch `scripts/train_causal_clean_plan_compiler.py`，用于Causal Clean-Plan Compiler Phase-A严格EAR瓶颈probe：约268万参数的temporal encoder/4-latent/action-noise decoder只能读取`final_teacher_2k`中的B6 student coarse EAR与共享action noise，禁止observation、IAR、task/episode旁路，并按相同seed7 episode-heldout 1794/206 split报告active前7维、full32、干预响应、same-task shuffled EAR和延迟。服务器现有PyTorch 2.7.0+cu126 wheel只包含至sm90 kernel，不支持RTX PRO 6000 Blackwell的sm120，首次GPU启动在任何训练step前以`no kernel image is available`退出；未安装新wheel，改用CPU完成一次诊断。1000步耗时552.83秒，全量active7 clean/response MSE为`0.0306909/0.0156750`，full32为`0.00673173/0.00343284`，response cosine为`0.37105`；same-task shuffled EAR active7 MSE升至`1.97334`，说明模型确实依赖EAR，但clean比B6 full32 `0.00499738`差约35%，response也未优于ridge `0.014660`或B6，故当前PyTorch attention compiler no-go。结果已通过飞书发送并得到`feishu_cpc_cpu_result_verified`。按用户要求，后续不再使用CPU训练，正在迁移为现有服务器已验证支持Blackwell的JAX/Flax JIT GPU版本，并并行准备attention和linear-residual两种compiler；本条记录时JAX版本尚未完成或运行。

2026-08-05（Asia/Shanghai）提交`5d8b617`新增GPU原生`scripts/train_causal_clean_plan_compiler_jax_mlp.py`，以JAX/XLA JIT取代不支持sm120的PyTorch CUDA路径，默认强制可见GPU且禁止静默回退CPU。服务器RTX PRO 6000 Blackwell上直接完成1000步正式训练，设备确认为`gpu:NVIDIA RTX PRO 6000 Blackwell Server Edition`，总耗时`27.36 s`，batch-1 JIT推理延迟mean/p95为`0.0968/0.1041 ms`，验证了GPU架构和Blackwell kernel可用。但约119万参数linear-transport+residual-MLP在1794/206 episode-heldout split上明显过拟合：最终active7 clean/response MSE为`0.0641724/0.0178272`，response cosine为`0.305995`，均未超过CPU attention或ridge对照；same-task shuffled EAR gap为`+1.91563`，说明严格EAR瓶颈生效但当前decoder泛化失败。因GPU速度已从552.83秒/轮降至27.36秒/轮，后续可快速比较架构；已增加held-out response主指标的best-checkpoint选择，并准备直接运行JAX/Flax attention compiler。

提交`11412ef`新增红682万参数的JAX/Flax BF16 attention compiler，并为attention与MLP版都加入每25步基于held-out response MSE、clean MSE tie-break的best-checkpoint保留。attention版在Blackwell GPU上1000步耗时`46.18 s`，选中step100，但完整held-out active7 clean/response MSE为`0.287520/0.020792`，response cosine`0.2286`，batch-1延迟`1.338 ms`，明显劣于MLP和ridge，因此attention no-go。随后利用GPU直接扫描了小容量/强正则MLP；有效配置中最好的width128/block1/response-weight3在约19秒内完成1000步，clean/response MSE为`0.077349/0.016564`，仍未超过ridge；三个`blocks=0`配置只在参数校验阶段退出，未进入训练。该结果支持停止普通flattened MLP/attention容量搜索。

提交`7d5b840`新增GPU原生`scripts/train_causal_clean_plan_compiler_jax_ridge_residual.py`：严格只用train episode split中的clean及有效semantic intervention对拟合冻结ridge base，再用小尺度residual MLP学习校正。独立闭式ridge扫描在`0.15 s`内完成，综合最优`lambda=0.01`的held-out active7 clean/intervention/response MSE为`0.0232044/0.0343845/0.0155368`，response cosine`0.367503`，shuffle gap`+1.94663`；response-only最优`lambda=1.0`虽为`0.0151992`，但clean/intervention恶化到`0.06605/0.07922`，未选用。三个ridge+residual GPU训练均在约19秒完成，最好response-weight3把response MSE从`0.0155364`降至`0.0152945`（约`1.56%`）且cosine从`0.36746`升至`0.36983`，但clean MSE从`0.0232044`退化到`0.0237923`（约`2.53%`），不足以进入闭环。

提交`7676160`新增严格小于300k参数的`scripts/train_causal_clean_plan_compiler_jax_temporal.py`，使用15→10时间插值、共享per-step MLP和depthwise kernel-3邻域混合，不使flattened sequence MLP或attention。首次启动因`scripts`不是Python package而在训练前`ImportError`退出，提交`fa89ecd`只修复该明确运行阻塞后直接重启。正式1000步耗时`18.53 s`，255,502参数，best step900；完整held-out clean/intervention/response MSE为`0.134749/0.159572/0.029266`，response cosine`0.26024`，batch-1 mean/p95延迟`0.3978/0.4281 ms`，同样no-go。CPC Phase-A的GPU架构更新已完成，训练一轮由CPU `552.83 s`降至JAX GPU `18.53–46.18 s`，但现有2k数据下EAR+action-noise-only的最佳整体方案仍是冻结ridge，所有神经compiler都不进入LIBERO闭环。下一步需要补充与部署图匹配的IAR/视觉条件或重新采集监督，而非继续EAR-only容量搜索。本阶段attention、MLP sweep与最终结论均已通过现有飞书脚本发送并得到`feishu_cpc_jax_attn_result_verified`、`feishu_cpc_sweep_verified`及`feishu_cpc_gpu_phase_a_final_verified`；数据盘剩余约`131 GiB`，无需扩容。

2026-08-05（Asia/Shanghai）实时复核westb服务器进度：本地与服务器代码均在`753e47a`，无CPC训练、LIBERO评测或policy server进程/tmux会话，RTX PRO 6000 Blackwell为`0 MiB/0%`，数据盘剩余约`131 GiB`。最新已完成结果未变：ridge+residual最好active7 clean/response MSE为`0.0237923/0.0152945`，还不足以进入闭环；当前阶段是已完成而非卡住，下一实验尚未启动。

2026-08-06（Asia/Shanghai）再次实时核对westb服务器：今日`00:00`后`ir_acot_pilot`下无新的`summary.json`，无训练、评测、policy server或tmux会话，GPU为`0 MiB/0%`，数据盘仍剩约`131 GiB`。本地与服务器在核对时均为`9fd65f9`，所有并行CPC子任务已完成；没有新指标可报告，IAR/视觉条件compiler与匹配部署图数据采集尚未启动。

2026-08-06（Asia/Shanghai）完成contextual causal plan compiler的真实在线接线与Task8闭环快速门。提交`332b4a7`新增`src/openpi/models/contextual_plan_compiler.py`并在`Policy`顺序Action-CoT路径中复用同一次VLM prefix、IAR、B6 1-step EAR和final flow noise，以约4.7万参数的JAX compiler替换final 300M expert；`policy_config.py`和`serve_policy.py`新增显式sidecar参数，未配置时原推理路径不变。服务器fast-forward到`332b4a7`后加载`final_b6/final/params`与`phase_b_contextual_branch_cpc_s7_e1a645a/model_params.npz`，磁盘仍剩约`131 GiB`。

正式闭环固定LIBERO-10 Task8、initial-state IDs 10–29、seed7、H10、EAR 1-step，共20局，结果位于`/root/autodl-tmp/acotvla/ir_acot_pilot/closed_loop_contextual_branch_cpc_task8_ep10_29_h10_seed7`。contextual compiler成功`16/20`，与历史10/10 teacher的`16/20`持平、比B6的`15/20`多1局、低于IR-lite的`17/20`；相对B6配对为5 rescues（10/13/15/20/24）和4 regressions（11/19/23/29），只有11/20 outcome一致，故净`+1`不足以证明质量提升。相对teacher为4 rescues和4 regressions，也只支持成功率未明显下降。其policy compute为`59.347 ms/call`，比B6的`65.206`快`8.98%`、比10/10 teacher约`83.641`快`29.05%`；RPC为`69.681 ms/call`，比B6快`9.74%`。成功回合完整elapsed为`11.452 s/episode`，只比B6快`2.35%`。因此该compiler形成有限计算收益，但没有显示稳定质量改进，也验证了只替换final suffix的端到端速度上限有限。实验开始和完成结论均通过现有飞书脚本发送并分别得到`feishu_task8_start_verified`与`feishu_task8_result_verified`；评测结束后服务器已停止、GPU释放。

同日按用户要求停止以MSE为主的结构盲扫并完成原论文机制审计。最相关的证据包括One-Step Flow Policy（OFP）的interval-average velocity、自一致性、自引导和previous-suffix warm-start，AdaFlow的条件方差自适应ODE步长，ParaDiGMS的Picard parallel-in-time，以及LLM speculative decoding/EAGLE的draft-and-batched-verify思想。下一主路线拟定为reasoning-conditioned hierarchical one-step flow：先只把final改为OFP式interval flow并以同一Task8 20局要求至少`17/20`；成立后再把EAR一并由10 NFE压到1 NFE并加入`Downsample(final)`与EAR的层级一致性，最后用AdaFlow式方差头按状态在1/3/10 NFE间细化。该设计保持执行H10，改变的是EAR/final两级连续动作生成和求解算法；当前只完成文献与repo可行性设计，OFP/AdaFlow训练代码及新闭环结果尚未产生。

2026-08-06（Asia/Shanghai）完成final-action OFP-SC快速验证与根因止损，相关提交为`c6b8043`、`8bc904b`、`26dbc1e`、`5ec7268`、`8720757`和`f40524f`。实现包含all-time flow anchor、EMA nested interval self-consistency、显式区间端点条件、可选previous-chunk warm start、full teacher endpoint anchor、弱区间条件、仅训练约213.2万final adapter参数及单位置编码的`time_blend`。纯OFP-SC从B6初始化训练600步耗时497.54秒，Task8 IDs10–29闭环为`0/20`且endpoint MSE约`3.0289`；加入full teacher endpoint anchor训练800步后EMA endpoint MSE降至`0.2170`，闭环仍为`0/20`。该证据表明局部self-consistency不能单独约束全局`1→0`精细操作映射，纯OFP-SC路线停止，后续必须保留绝对teacher endpoint/distribution监督。

同日发现无需训练的弱区间条件是有效动作校准：加载原`final_ir` sidecar、`half_concat α=0.05`时Task8为`19/20`，但policy为`72.7186 ms/call`；改用单位置编码`time_blend α=0.05`后为`18/20`、`74.9216 ms/call`，说明动作质量可保留但OFP专用请求/JIT入口带来结构性额外延迟。提交`5e16099`把`r=0`的同一时间校准直接并入原IR单步final expert，20局为`17/20`、`67.0114 ms/call`；提交`7e64fef`再增加静态α的direct one-step图，移除动态`lax.while_loop`后同协议达到`18/20`、`66.1081 ms/call`，失败IDs为17和29。direct候选相对历史IR `17/20、65.4527 ms/call`净增1次成功且保持相近单次计算，数值质量/速度门已通过，但固定`α=0.05`只定位为强baseline，尚不作为ICRA主算法贡献。下一步正在实现初始严格等价该baseline的state/prefix-conditioned零初始化时间校准与teacher-anchored训练，再加入按风险升级1/2/4/10 NFE的回退；上述各轮开始与完成均通过现有飞书脚本发送并返回验证成功。

2026-08-06（Asia/Shanghai）完成opt-in `adaptive_final_time_warp`代码实现。`ACOTConfig`新增默认关闭开关；final expert前新增零初始化线性gate，输入为池化prefix与归一化state，输出`alpha=0.05+0.05*tanh(logit)`，初始逐输入等价固定`alpha=0.05`，推理只调用一次final LLM。`train_acot_endpoint_distillation.py`支持只训练gate、从`final_ir` sidecar恢复教师锚定分支、使用endpoint+IR loss并保存`final_ir+gate`组合sidecar；serving会检测sidecar gate参数并启用对应model config，policy和LIBERO eval增加默认关闭的自适应direct one-step开关。本次按要求未运行测试、训练或commit，暂无新指标结果。

随后在westb RTX PRO 6000 Blackwell上完成两轮2,081参数gate训练，数据均为`final_teacher_2k`的episode-heldout 1,794/206 split，初始化均为`final_ir`，只用一次final LLM的endpoint+IR目标。高学习率`3e-4→3e-5`训练300步耗时280.33秒，gate在约50步后饱和，full-validation alpha mean/std为`0.00004392/0.00001408`、final MSE为`0.00496817`，实质退化到未校准alpha≈0，判no-go且未跑闭环。温和学习率`3e-6→3e-7`训练600步耗时423.66秒，alpha mean/std为`0.05093725/0.00004437`，几乎仍是固定常数，full-validation final MSE反而为`0.00813485`，同样不进入闭环。两轮说明仅用平均endpoint+IR损失会让bias吸收梯度，无法学出有意义的state/prefix router，而且10-step teacher MSE与固定alpha=0.05的Task8闭环收益方向不一致；下一步改为显式导出`{0,0.025,0.05,0.075,0.10}`逐样本前7维风险网格，以episode-disjoint对比/风险监督训练router，不再继续调同类学习率。两轮开始和no-go结论均已通过飞书脚本发送并验证。

同日补跑固定`alpha=0.025`的direct one-step final time-warp，协议保持Task8、initial-state IDs 10–29、seed7、H10、EAR NFE=1。结果为`18/20`，失败IDs 10和22，policy为`65.6521 ms/call`、RPC为`78.0330 ms/call`；与`alpha=0.05`同为18/20但失败IDs 17和29不同，说明不同状态对校准强度存在互补偏好。该固定参数仍只作为强baseline；当前证据支持转向same-snapshot、task-outcome-supervised的状态级time-warp路由，而不是继续以teacher MSE训练平均gate。本轮结果已通过现有飞书脚本发送并输出`FEISHU_VERIFIED`。

随后完成`final_teacher_2k`的五路`alpha={0,0.025,0.05,0.075,0.10}`GPU风险导出与episode-disjoint线性路由诊断，outer holdout严格为206条。按前6维Huber与gripper符号加权的teacher-action风险，oracle分布为alpha0 139条、0.025 65条、0.05仅2条；oracle相对alpha0平均gain仅`0.00007431`。五风险router top1为`52.91%`、top2 coverage为`82.52%`，但realized risk `0.00678358`劣于固定alpha0的`0.00639807`；拟合现有标量gate虽top1为`67.96%`，却在206条中205条选择alpha0，仅1条选择0.025。该结果再次证明teacher动作风险与Task8闭环中alpha0/0.025/0.05的成功率互补不一致，因此本监督路线判no-go，不进入闭环；结果位于`/root/autodl-tmp/acotvla/ir_acot_alpha_router_s7`，完成结论已通过飞书发送并验证。

同日新增并运行Task8同物理快照alpha反事实progress pilot，输入为既有counterfactual数据的非formal episodes 0–9，SHA去重后29个MuJoCo root；每个root以同一stored policy seed分别生成alpha0与alpha0.05的direct one-step H10动作，再以共同alpha0控制策略和绝对step seed续跑到H20。采集耗时`171.97 s`、平均两臂action RMSE为`0.03158`；H10 progress为alpha0 wins 5、alpha0.05 wins 4、ties 20，H20为8/2/19，说明两方向都存在但短时progress总体偏alpha0。由于fixed-alpha正式闭环的失败集合互补，已启动同29 roots的terminal-success升级以获得任务终局主标签；本条记录时terminal采集仍在运行，尚无终局结果。

上述29-root数据随后完成terminal-success升级，耗时`708.42 s`。alpha0.05相对alpha0出现3次terminal rescue、3次terminal regression和23次success tie；按terminal success优先、both-success再比较remaining calls/steps、both-fail再比较terminal privileged progress的词典序偏好，alpha0.05 wins 15、alpha0 wins 14、无tie。该结果直接证明同一Task8中固定两种one-NFE query time存在双向、state-dependent的任务终局headroom，但只有6/29 roots提供非tie success监督，尚不能证明路由器可学习；下一步正在以episode分组leave-one-episode-out评估强正则低容量outcome router，formal IDs10–29不会进入训练。终局结果已通过飞书发送并验证。

29-root outcome router的严格episode-group leave-one-episode-out结果随后完成。高维`pooled prefix2048+state32`路由整体balanced accuracy/AUROC仅`0.3833/0.2905`，说明小样本直接使用VLM prefix严重过拟合；低容量`normalized state32+absolute decision step`路由整体达到`0.7571/0.7452`，alpha0与alpha0.05两侧recall为`0.7143/0.8000`。在仅6条terminal success-discordant roots上balanced accuracy为`0.6667`，样本仍很少。按out-of-fold选择回放，compact router的terminal success为`24/29`，比两个固定alpha的`23/29`各多1，hindsight oracle为`26/29`；这是development same-root证据，不等同formal闭环。结果位于`/root/autodl-tmp/acotvla/alpha_snapshot_pairs/task8_dev_outcome_router_s7`；下一步需要部署compact state+step路由并只在未用于训练的IDs10–29上评测。

独立final NFE headroom也完成快速探针与完整闭环。alpha0.05、final NFE2首先在NFE1失败IDs17/29上`2/2`救回；alpha0.025、NFE2在其失败IDs10/22上救回ID10但ID22仍失败。随后alpha0.05 NFE2完整Task8 IDs10–29为`19/20`，唯一失败ID13，policy为`69.4247 ms/call`；相对alpha0.05 NFE1的`18/20、66.1081 ms/call`净增1成功但慢约3.3 ms，并出现NFE1未有的ID13 regression。该结果把NFE2定位为质量headroom或经置信校准的少量fallback，不能全局替换one-NFE，也不作为主算法创新；结果与结论已通过飞书发送并验证。

2026-08-06（Asia/Shanghai）完成compact outcome router首次未见formal闭环。模型仅使用归一化state32与absolute decision step，在每次final action前硬选择direct one-NFE `alpha=0`或`0.05`；训练数据仅来自Task8非formal episodes 0–9的29个same-snapshot terminal pairs，正式评测固定Task8、initial-state IDs 10–29、seed7、H10、EAR NFE1，共20局。结果为`17/20`，失败IDs为20、21、23，低于固定alpha0.025与alpha0.05各`18/20`，仅追平alpha0的`17/20`；1032次决策中alpha0/0.05分别为845/187，说明formal状态分布下router明显偏向alpha0。汇总policy compute为`77.2264 ms/call`，包含首回合第二个静态alpha分支的JIT编译，仍需去除首局后审计稳态开销；当前实现还没有写出router自身计时。该版本质量与速度均判no-go，不在formal结果上调阈值；下一轮转向EAR/IAR计划特征探针、非formal on-policy困难状态聚合、轨迹一致性/置信校准以及消除JAX device-to-host同步。结果位于`/root/autodl-tmp/acotvla/alpha_snapshot_pairs/task8_formal_compact_outcome_router_s7`，no-go与下一步已通过飞书发送并得到`FEISHU_SEND_VERIFIED`。

同一formal结果去除整个首局ID10及其第二个静态alpha图JIT后，995次稳态调用的policy/server/client-RPC wall分别为`72.9499/83.9665/85.9907 ms/call`；即使不计JIT，policy仍比固定alpha0.025与0.05慢约`11.12%/10.35%`。post-JIT policy分解均值为VLM `36.975`、IAR `2.042`、EAR `3.909`、final expert `3.612`及profile overhead `26.411 ms`；失败局与成功局的alpha0.05覆盖率仅`17.51%`与`18.37%`，聚合上几乎无区分。该审计进一步确认当前Python/NumPy router触发了不合理的设备同步/计时开销，需移入JAX图或改为无同步原始输入路径。

随后完成29-root pre-final Action-CoT特征的严格nested leave-one-episode-out探针，结果位于`/root/autodl-tmp/acotvla/alpha_snapshot_pairs/task8_dev_plan_feature_probe_s7`。比较的特征为state+step、state+EAR summary、state+IAR mean的fold-train PCA、EAR+IAR及prefix的fold-train PCA；PCA、标准化、超参选择均只在每个fold训练侧拟合，且不读取候选final action或outcome作为输入。state+step整体BA/AUROC为`0.7571/0.7452`；state+IAR为`0.7238/0.7929`，其余EAR、EAR+IAR及prefix也未在预注册排序上超过state+step；各方案OOF终局回放均未超过`24/29`。因此`beats_state_step_under_prespecified_metric_rank=false`，当前小样本下直接堆EAR/IAR summary或PCA无益，不部署新的plan-feature sidecar；no-go结论已通过飞书发送并得到`FEISHU_PLAN_PROBE_VERIFIED`。

完成compact alpha router部署延迟的静态路径审计与低开销改造。原LIBERO管线中的state在`LiberoACOTInputs -> Normalize -> ACOTPadStatesAndActions`后仍为host NumPy，因此原`np.asarray(inputs["state"])`本身不构成已证实的device-to-host复制；更明确的开销源是每次请求把三个CPU路由诊断标量分别构造成JAX device array，以及alpha0/0.05作为静态参数产生两个final endpoint JIT图。新实现把已验证的33维ridge affine、阈值选择和dynamic alpha直接融合进endpoint-student final JAX图，router kernel/bias在policy构造时只上传一次，score/alpha随action在同一图中输出；这样两个alpha共享一个可执行图，消除第二分支首次编译和逐请求独立标量dispatch/sync。默认与固定alpha路径保持不变，只有同时加载router sidecar并显式请求`action_cot_compact_alpha_router`时启用。已通过`py_compile`和`git diff --check`静态检查，未运行本地训练或闭环，实际稳态延迟仍需服务器复测。

随后fused JAX router的服务器formal复测虽然把policy降低到约`68–71 ms/call`，但出现旧CPU/static路径成功而fused路径失败的ID11，证明dynamic-alpha融合改变了动作数值路径，不能作为等价延迟优化。代码因此退回原float64 CPU score/threshold与alpha0/0.05两张static direct endpoint图；低风险优化只保留在主模型device-get完成后直接注入host NumPy score/alpha/step，避免每请求构造三个诊断JAX小数组。compact模式首次请求用同一prefix/EAR/IAR安全计算并预热两张static final图，返回对应alpha的原图输出，后续切换不再在正式episode中触发第二次编译；默认及固定alpha路径不启用该预热。fused model入口已移除，router NPZ的已验证raw affine字段继续保留。代码通过`py_compile`与`git diff --check`，本条记录时尚未完成回退版服务器闭环复测。

融合JAX路由随后在完全相同Task8 formal IDs10–29、seed7、H10、EAR NFE1协议下完成20局，结果位于`/root/autodl-tmp/acotvla/alpha_snapshot_pairs/task8_formal_compact_outcome_router_fused_s7`。fused router为`19/20`，唯一失败ID11；969次决策中alpha0/0.05为812/157，policy/server/RPC wall分别为`69.1407/80.8270/82.3520 ms/call`。相较旧CPU/static-router的`17/20`及去首局后`72.9499 ms/call`，本轮同时多2个成功且policy快约`5.22%`；相较固定alpha0.025/0.05的`18/20、65.6521/66.1081 ms/call`，多1个成功但仍慢约`5.31%/4.59%`，尚未达到约67 ms目标。需要注意fused dynamic-alpha图改变了XLA浮点执行路径：旧路由成功的ID11在本轮回退，而旧失败20/21/23被救回，因此这次seed7提升不能只归因于相同离散路由决策，也不能在独立seed复现前当成稳定算法收益。结果与上述限制已通过飞书发送并得到`FEISHU_FUSED_RESULT_VERIFIED`。

同一fused router随后在独立policy seed107、其余formal协议不变时得到`17/20`，失败IDs为12、17、23，policy/server/RPC wall为`67.3090/78.3726/79.8447 ms/call`；结果位于`/root/autodl-tmp/acotvla/alpha_snapshot_pairs/task8_formal_compact_outcome_router_fused_s107`。seed7与107合并为`36/40`，高于此前未路由IR-lite两seed的`34/40`，但单seed从19降到17说明方差明显，且seed107速度仍略高于约67 ms目标，不能据此定稿或声称稳定优越。独立seed结果已通过飞书发送并得到`FEISHU_FUSED_S107_VERIFIED`；下一步保持formal IDs隔离，使用episodes30–49的current-router on-policy canonical snapshots采集首chunk反事实、后续共享当前policy的`Q^pi`终局标签。

2026-08-06（Asia/Shanghai）完成HARP（Hierarchy-Aligned Residual Projection）最小GPU pilot的可训练、可部署代码实现，尚未运行训练或闭环。新增GPU-only exact pair exporter：加载base checkpoint与现有`final_ir` sidecar，复用endpoint 2k数据中的canonical observation及stored coarse/final noise；新opt-in模型入口共享同一个`t=1` velocity，以两次final suffix严格生成`A1=z-v1`和`A2=(z-0.5v1)-0.5v(x0.5,0.5)`，保证A1/A2共享observation、EAR NFE1、IAR与noise。pair HDF5保存state、EAR、IAR、noise、A1/A2及episode标识，预计主体为float16 IAR缓存。

新增约8.1k参数的JAX/Flax temporal residual head及trainer，输入包括A1、按物理时间线性对齐的EAR、A1-EAR、A1/EAR temporal deltas、final noise、state前6维和IAR统计；输出前6维residual、heteroscedastic log-variance与EAR reliability，gripper及其余维度不修改。trainer强制GPU、训练步数小于10000，采用task/episode-disjoint split；held-out partition同时校准confidence temperature/bias与p95 residual trust projection，并保存严格schema的`model_params.npz`、validation指标和batch1 sidecar p95 latency。`policy_config.py`、`serve_policy.py`、`Policy`与LIBERO eval增加双重opt-in路径：server必须显式加载HARP NPZ，请求还必须发送`action_cot_harp_residual=True`；修正直接使用device上的IR NFE1/EAR/IAR/noise/state JAX arrays，禁止与router、time warp、OFP、compiler、MC或非NFE1组合，默认推理不变。本次按用户要求未运行本地测试、训练或commit，尚无HARP质量/速度结果。

同日完成Task8非formal episodes30–49的current-router on-policy `Q^pi`采集与严格episode隔离验证。采集固定seed7、H10、EAR NFE1；每个行为episode均匀选择early/middle/late三个canonical MuJoCo roots，候选首chunk分别使用alpha0与alpha0.05，随后双方都由冻结的旧compact router继续执行，并复用absolute-step seeds。20局行为成功`15/20`；60个roots中alpha0.05相对alpha0为3次terminal rescue、1次terminal regression和56次终局tie，总偏好为alpha0.05 wins 22、alpha0 wins 21、ties 17，耗时1119.22秒。训练只使用episodes30–39的30 roots，其中有2 rescues、0 regressions；compact state32+absolute-step ridge的episode-LOO整体BA/AUC仅`0.5917/0.6167`，且两个terminal rescues均预测错误，OOF终局为`26/30`，低于固定alpha0.05的`28/30`。

冻结该router后只在held-out episodes40–49的30 roots回放，terminal-discordant为1 rescue/1 regression；router terminal-discordant accuracy为`0.5`、整体decisive BA为`0.6045`，router、固定alpha0和固定alpha0.05的终局均为`20/30`，但router的成功root remaining calls/steps均弱于固定alpha0.05，hindsight oracle也仅`21/30`。该结果判定二值alpha状态路由no-go：真实flow-time修正确有少量双向headroom，但状态路由不能稳定辨别，且held-out净终局收益为0。采集、训练和验证结果分别位于`/root/autodl-tmp/acotvla/alpha_snapshot_pairs/{task8_onpolicy_qpi_ep30_49_staticpi_s7_a0bdb98,task8_onpolicy_qpi_router_train30_39_s7,task8_onpolicy_qpi_router_heldout40_49_s7}`；最终结论已通过飞书发送并得到`FEISHU_QPI_RESULT_VERIFIED`。GPU服务随后停止，服务器代码快进到`1414570`，下一步转向HARP逐时刻逐维NFE2残差投影。

2026-08-06（Asia/Shanghai）在contextual compiler与B6 direct one-step expert的同请求双分支路径上新增未提交的动态局部融合实现，修改`src/openpi/policies/policy.py`与`scripts/eval_libero_execution_horizon.py`。opt-in `semantic_gate`复用同一observation、EAR、IAR与final noise下已经生成的两块动作，不增加模型调用；它按归一化translation 0:3、rotation 3:6的逐时刻组RMS分歧生成带一帧邻域的平滑门，按gripper符号冲突、符号转折和低幅margin生成离散接触事件门，并允许`high_disagreement_source=expert|compiler`比较两个有向假设。eval CLI透传translation/rotation/gripper阈值和gate width，决策与summary记录translation/rotation平均分歧、gripper conflict rate、expert gate rate及融合耗时；默认仍为compiler，不启用双路或门控。

同一未提交改动另加入只用于互补性诊断的`phase_compiler_expert`与`phase_expert_compiler`：eval每次透传absolute environment step及默认400的switch step，server在阈值前后只运行相应单一分支，不承担双路时延；warmup会在switch两侧各请求一次以避免正式回合中首次编译另一分支。phase响应回传并由eval核对实际step、switch和expert选择率。按要求未运行本地训练、单元测试或闭环；`git diff --check`未发现空白错误，动作质量、门控覆盖和实际速度尚无实验结果。

2026-08-06（Asia/Shanghai）补充完成逐token/坐标级非均匀diffusion/flow时间与phase/contact-aware机器人控制的主来源调研。NeurIPS 2024 Diffusion Forcing直接训练独立per-token噪声等级并允许共享模型按任意逐token schedule去噪；ICML 2026 Self-Flow在flow matching中公开实现per-token timestep conditioning，这两项共同支持把标量时间改成`[B,H]`时间向量并在一次backbone forward中输出各token对应的velocity，但都不意味着从纯噪声一次NFE即可得到最终动作。ICRA 2025 FMNS workshop的Rolling Diffusion Policy是最接近机器人action chunk单步生成的直接先例，但依赖滑窗rolling状态、仅属workshop证据；NoiseGate用独立per-latent时间和task-reward优化的gating policy支持“学习调度而非手调超参”，但其对象为未来视频latent而非action token。PACE以动作速度低谷识别操作phase边界，FoAR以future-contact probability区分接触阶段，可作为逐token预算/时间分配信号，但本身不支持逐token flow或one-call结论。设计上的必要条件是训练期就采样异质时间profile并逐token注入时间embedding；若目标是纯噪声到最终动作的一次NFE，还需额外的一步consistency/endpoint distillation目标，不能仅凭逐token时间文献声称可行。

2026-08-06（Asia/Shanghai）完成token-wise非均匀时间条件final flow的实现、消融与双seed Task8正式验证，提交依次为`6402a82`、`4d67388`、`296ca41`、`ca917d5`、`6a3d1c6`和`02a0203`。该opt-in路径保持H10、EAR NFE1与final NFE1，不增加第二动作分支或第二次final Transformer调用；区别是把原本整个action chunk共享的标量flow time替换为长度10的逐token时间向量，并在pi0.5 expert的AdaRMS中逐action token注入。最终静态square-root ramp对应alpha为`[0.025,0.033333,0.036785,0.039434,0.041667,0.043634,0.045412,0.047048,0.048570,0.050]`；XLA在JIT边界专门化该schedule，服务端输出树改为一次`jax.device_get`后再转NumPy，默认未启用路径保持原有行为。

最终同协议均为LIBERO-10 Task8、initial-state IDs 10–29、H10、EAR NFE1、final NFE1。seed7结果`18/20`，失败episodes 15/20，policy/server/RPC wall为`58.0162/67.2828/68.8180 ms/call`，位于`/root/autodl-tmp/acotvla/tokenwise_timewarp/task8_formal20_sqrt_ramp_s7_v10_deviceget/summary.json`；独立policy seed107同样为`18/20`，失败episodes 10/15，policy/server/RPC wall为`61.6131/72.1331/73.6401 ms/call`，位于`/root/autodl-tmp/acotvla/tokenwise_timewarp/task8_formal20_sqrt_ramp_s107_v10_deviceget/summary.json`。因此该版本达到预设的每seed至少`17/20`且policy约不高于`67 ms/call`快速门。此前linear ramp为seed7 `20/20`、seed107 `16/20`；binary step2、step5和compressed ramp5均只在seed7快速门得到`2/4`，exact linear 4-token探针为`4/4`而rank-2近似仅`2/4`且更慢，这些消融支持保留覆盖完整chunk的连续、精确异质时间路径。个别边界episode在不同服务重启/完整运行间存在结果波动，因此双seed正式结果可作为当前可行性证据，尚不能声称统计显著优于所有固定alpha基线。

当前算法层面的可行路线是“同一次flow/Transformer调用内的逐token异步时间条件”，而非继续选择单个全局H、K或alpha；静态square-root schedule仍属于proof-of-concept。若作为ICRA主方法，下一步需要在训练期采样异质时间profile，加入逐token endpoint/consistency distillation，并用phase/contact或任务回报学习schedule，再补Task8多训练seed及Task3/9或全任务配对成功率和端到端延迟。双seed完成结果已调用现有`scripts/watch_experiment_feishu.py`发送，命令退出码为0；本地`main`已通过代理推送到GitHub。

2026-08-06（Asia/Shanghai）进一步完成当前token-wise final time calibration的原理与证据边界复核。当前数学路径不是逐token Euler步长：所有动作位置仍以同一纯噪声状态`x(1)=epsilon`作为输入，只把条件时间改为`tau_i=1-alpha_i`，再用一次final suffix前向统一输出`a_hat_i=epsilon_i-v_i(epsilon_1:T,tau_1:T;observation,EAR,IAR)`；chunk内attention使每个velocity依赖整条动作和整条time profile。逐token正余弦time embedding经final time MLP后，在pi0.5每层AdaRMS中分别产生scale、shift和residual gate；当前square-root schedule为`alpha_i=0.025+0.025*sqrt(i/9)`，条件时间从0.975降到0.950。该实现保持EAR NFE1、final NFE1和一次final Transformer调用，但存在训练时标量time、推理时异质time以及`x(1)`与`tau_i<1`不一致的OOD time-state mismatch，因此更准确地属于推理期endpoint校准，而不是严格异步flow积分。

效果复核的严格结论为：square-root在Task8相同20个initial states上的policy seed7/107均为`18/20`，合计`36/40`；linear ramp同样合计`36/40`但分布为`20/20+16/20`，所以square-root只显示更均衡的worst-seed表现，未提高总成功数。相对fixed alpha0.05两seed合并仅从`35/40`到`36/40`，配对为5 rescues/4 regressions、McNemar exact双侧`p=1.0`，不能声称统计显著优越。58.02/61.61 ms是policy compute而非端到端wall；server/RPC仍为67.28/68.82和72.13/73.64 ms。最终v10还包含全输出树一次`jax.device_get`的系统优化，旧fixed-alpha尚未在同一v10上重跑，因此不能把约12%的表观延迟下降归因于token-wise算法。真正的训练升级应按`x_i(t_i)=t_i*epsilon_i+(1-t_i)*a_i`采样异质time profile，并在部署输入`x_i=epsilon_i`上加入one-step endpoint/consistency distillation，再由phase/contact-conditioned learner产生time schedule；验证需使用未参与调参的新initial states、Task3/9或全LIBERO-10以及多个training seed。

2026-08-06（Asia/Shanghai）完成摆脱人工token schedule并形成ICRA主方法的下一阶段设计。近期NoiseGate已经覆盖“独立per-latent时间训练、轻量schedule policy和GRPO任务奖励”，因此仅用MLP/GRPO把静态sqrt替换为10维action时间会有明显的迁移式创新风险。推荐主线暂称PACT-Flow/PlanFlow-CoT：先把EAR按现有15→10物理时间与归一化映射得到动作锚点`P`，由读取observation、EAR、IAR、state和token position的轻量scheduler生成每状态逐token时间`t_i`，构造训练部署一致的plan-anchored bridge state `x_i=t_i*epsilon_i+(1-t_i)*P_i`，再用同一次final Transformer输出velocity并执行`a_hat_i=x_i-t_i*v_i`。该设计使时间同时改变输入transport state、AdaRMS条件和逐token积分区间，消除当前纯噪声`x(1)`配`t_i<1`的mismatch；最终仍保持EAR NFE1、final NFE1和一次final suffix调用。

最短训练分三步：先复用现有`final_teacher_2k`在线合成真实endpoint插值的随机异质时间样本，训练vector-time flow；再用EAR anchor与10-NFE teacher endpoint联合训练scheduler和final 300M expert，endpoint loss只负责安全初始化；最后冻结大模型，扩展现有same-snapshot collector，从scheduler连续分布采样多组schedule并按terminal success、完成成本及失败progress生成配对偏好，只更新scheduler做continuous-DPO/AWR，使真实任务结果而非teacher MSE决定时间场。首轮600–1000步预计约8–25分钟、新sidecar约2 GiB；先在未参与调参的新Task8 IDs上以两个policy seed、同版本fixed scalar/static sqrt对照做40局，要求至少`37/40`、rescues严格多于regressions且scheduler开销不超过约3 ms后，才扩到Task3/9与全LIBERO-10。正式论文主张应是Action-CoT cross-level reliability驱动的one-step action transport，而不是首次逐token timestep、首次learned schedule或逐token计算量分配；所有dense token实际FLOPs仍相同。

2026-08-06（Asia/Shanghai）完成PACT-Flow（Plan-Anchored Cross-level Token Flow）首版代码接入，尚未开始训练或闭环评测。新增`src/openpi/models/pact_flow_scheduler.py`，以约14.7万参数的EAR-IAR cross-attention scheduler联合输出逐动作token时间`tau`、受异方差残差监督的置信度以及学习到的action-space计划锚点；模型侧按既有EAR物理时间约定对齐15步粗计划到10步final chunk，构造`x_i=tau_i*epsilon_i+(1-tau_i)*P_i`，在同一个final JIT图内只调用一次final expert并得到`a_hat_i=x_i-tau_i*v_i`。训练损失在一个batch的一次final forward中混合matched heterogeneous vector-time flow、matched scalar-time flow和learned plan-bridge endpoint三种状态，同时监督plan anchor、uncertainty calibration及schedule smoothness；训练范围为B6 final expert与scheduler，VLM/EAR/IAR保持冻结。

推理与评测均为双重opt-in：sidecar必须完整包含`pact_flow_scheduler/.*`并通过严格shape/path检查，请求还必须显式发送`action_cot_pact_flow_scheduler=True`；默认checkpoint、原policy和原eval路径不改变。PACT路径禁止与静态token time、scalar/adaptive warp、OFP、router、HARP、contextual compiler、MC/coupled sampler及其他final refinement组合，首轮协议固定EAR NFE1、final NFE1和H10；动态`tau`不是JIT static arg，不会因数值变化重新编译，且与actions统一取回而不增加单独device barrier。新服务器`ssh -p 25296 root@connect.westb.seetacloud.com`已确认RTX PRO 6000约98 GiB显存空闲、JAX识别GPU、数据盘剩余118 GiB，因此本轮无需扩容；训练与Task8实际结果尚不可用。

同日首次直接启动600-step PACT GPU pilot时，base 50999与B6 sidecar均成功恢复，但在step 1之前的`init_train_state`退出，错误为`ValueError: pytree structure error: different pytree metadata at key path pjit out_shardings.model_def`，没有写出checkpoint或训练指标。根因是scheduler使用`jax.nn.initializers.constant(...)`创建的闭包作为NNX Linear静态GraphDef字段，两次model create得到不同函数对象；修复为tau/log-std两个head均使用稳定的zero initializer，并在forward中显式加初始tau logit和初始log-std常数，保持数值初始化不变。该修复后需重新启动原600-step训练确认运行时结果。

修复后600-step full-final PACT训练在`/root/autodl-tmp/acotvla/pact_flow/pact_b6_s7_600_r2`完成，包含首次编译共393.93秒，输出组合sidecar约1.6 GiB、保存参数860344034个，训练范围为430245570个final expert与scheduler参数。全验证206条中bridge endpoint MSE为`0.00494076`，但不同随机mode混合使整体primary loss不可直接与旧B6 endpoint MSE横比；更关键的是`tau_mean=0.975019`、`tau_std=5.87e-5`、`tau_min/max=0.974910/0.975107`，plan-anchor RMS仅`0.010758`，说明共享`3e-6`学习率下14.7万参数scheduler几乎仍停留在统一初始时间场。基于该证据未浪费Task8 20局闭环预算，新增默认关闭的`--pact-scheduler-only`两时间尺度阶段：从完整PACT sidecar恢复并严格校验scheduler子树，冻结final expert/VLM/EAR/IAR，仅以较高学习率训练scheduler；下一轮将把heterogeneous/scalar概率均设为0，使全部样本直接优化部署一致的plan-bridge endpoint。

2026-08-06（Asia/Shanghai）完成PACT scheduler-only续训与Task8正式20局闭环。scheduler-only从full-final PACT sidecar恢复，只更新147106个scheduler参数，使用bridge-only目标和`3e-4→3e-5`学习率训练1000步，包含编译共368.65秒；输出位于`/root/autodl-tmp/acotvla/pact_flow/pact_sched_s7_1000_lr3e4`。206条全验证中plan-anchor MSE由约`0.223`降至`0.0089113`，置信度残差RMS校准到`0.913`，但bridge endpoint MSE仅由`0.0049407606`变为`0.0049407068`，`tau_mean/std/min/max=0.974593/0.000255/0.974128/0.975125`，仍接近统一时间场。

正式闭环固定LIBERO-10 Task8、initial-state IDs 10–29、policy seed7、H10、EAR NFE1和final NFE1，使用上述scheduler-only sidecar，结果为`15/20`，失败IDs为11、13、14、17、21；结果位于`/root/autodl-tmp/acotvla/pact_flow/task8_pact_s7_20/eval`。policy/server/RPC wall分别为`63.5619/74.9229/76.5016 ms/call`，实际rollout中的`tau_mean/std/min/max=0.974559/0.0000505/0.974490/0.974650`。该成功率只追平历史B6的`15/20`，低于IR-lite的`17/20`及静态square-root token time的双seed各`18/20`；因此当前teacher-MSE/bridge监督的PACT明确判为no-go，不继续增加同类训练步数或调学习率。新服务器`ssh -p 25296 root@connect.westb.seetacloud.com`确认可直接复用所有checkpoint与数据，RTX PRO 6000约98 GiB，数据盘剩余约115 GiB；PACT评测服务已停止，GPU释放，无需扩容。

2026-08-06（Asia/Shanghai）完成严格opt-in的Temporal Prefix Reuse可行性pilot代码，尚未运行训练或闭环评测。`Policy`按请求中的reuse period与absolute decision step缓存最近一次完整VLM `prefix_state`；period=2时按fresh/reuse/fresh交替，复用请求只沿用旧`prefix_tokens/prefix_mask/prefix_out/KV`，当前preprocessed observation/state以及coarse/final flow noise均重新生成，并照常重跑IAR、EAR和final。absolute step为0、回退、相等或period改变时强制清空并刷新；默认period=0会清空缓存且走原推理路径。

`scripts/eval_libero_execution_horizon.py`新增`--temporal-prefix-reuse-period`并透传absolute step，记录每decision的`vlm_prefix_reused`与episode内累计`vlm_prefix_refresh_count`，同时汇总reuse/refresh call数及reuse rate。首版只允许profile sequential endpoint EAR/final NFE1，禁止PACT、batched MC、predictor/selector、joint、dynamic denoising、adaptive/OFP/router/HARP/contextual compiler及其他复杂final分支；为与当前最强基线配对，允许固定10值`final_token_time_warp_alpha`的static-square-root one-step final路径。按要求未运行训练、单元测试或smoke；仅完成`py_compile`与`git diff --check`，真实加速、Task8成功率及contact/gripper回归尚无结果。当前缓存为policy进程内单客户端pilot状态，依赖absolute step判断episode边界，并发客户端隔离仍未实现。

同日完成Temporal Prefix Reuse的Task8正式20局快速门，提交为`624d8cb`，服务器输出位于`/root/autodl-tmp/acotvla/p3t_prefix_reuse/task8_sqrt_period2_s7_20`。协议固定initial-state IDs 10–29、policy seed7、H10、EAR/final NFE1，并叠加当前static-square-root逐token time；1:2路径实际复用735次、刷新749次，reuse rate为`49.53%`。结果仅`11/20`，失败IDs为11、12、13、15、17、19、20、22、29；相对相同IDs的full-fresh static-square-root `18/20`为0 rescue、7 regressions、11 both-success和2 both-failure，质量明确no-go。

含首次reuse图编译的summary为policy/server/RPC `61.6909/72.8996/74.4124 ms/call`。排除大于500 ms的两次首次编译后，同运行fresh calls平均`64.6215 ms`、其中VLM `38.5320 ms`；raw cached calls平均`46.9809 ms`、其prefix准备仍为`20.7854 ms`；1:2稳态均值为`55.8904 ms/call`，相对同运行全刷新约降低`13.5%`，但无法补偿成功率从90%降到55%。该证据支持停止调raw reuse period，并把它只保留为后续方法必须超过的系统下界；下一主线改为Plan-Conditioned Predictive Prefix Transport（P3T-ACoT）：在非关键帧保留当前低分辨率视觉/state，利用已执行动作与剩余EAR预测逐层prefix语义的低秩transport，再重新生成IAR/EAR/final，并以fresh-action regret训练cache-risk verifier。创新不能表述为cache/gating本身，而应验证Action-CoT驱动的predictive semantic transport；raw reuse完成/no-go结论已通过现有飞书脚本发送，评测与policy服务随后停止、GPU释放。

随后完成P3T-Lite首版可训练与可部署代码，尚未产生训练或闭环指标。新增约9.36万参数的固定形状`src/openpi/models/p3t_prefix_transport.py`：输入anchor/current两路64×64图像、归一化state差、10步已执行动作和anchor EAR，对18层、968-token、单KV-head prefix cache执行视觉token inverse-RoPE→双线性warp→RoPE，并对视觉512 token和language 200 token施加6组rank-4计划条件低秩K/V残差；masked dummy 256 token保持不变，独立sidecar采用带SHA256/shape/path校验的`config.json+params.npz`。新增`scripts/train_p3t_prefix_transport.py`，从`windows2k`的Task8映射HF `task_id=6`筛选anchor→anchor+10的episode-disjoint pairs，从现有12 GiB Arrow cache精确恢复原256图像、确定性224模型观测和prompt，在线生成anchor/fresh KV而不落盘全量cache，并以delta-normalized KV、IAR、EAR、同noise action及risk联合监督冻结base之外的adapter。`Policy`、`policy_config.py`、`serve_policy.py`及LIBERO eval增加严格双重opt-in P3T sidecar路径；仅允许period2与EAR/final NFE1，keyframe末缓存normalized EAR/action，fast frame transport KV后重新运行IAR/EAR/final，默认推理不变。新服务器RTX PRO 6000约98 GiB显存空闲、数据盘剩余约115 GiB；全量KV预计超过117 GiB且不会保存，当前无需扩容。按效率要求未运行单元测试或额外smoke，下一步直接启动500-step GPU训练，以held-out action/semantic gap closure和adapter延迟决定是否进入Task8 20局。

P3T-Lite在新服务器完成500-step GPU训练，输出位于`/root/autodl-tmp/acotvla/p3t_prefix_transport/task8_stride10_s7_500_dd24cfe`。首次启动因trainer的Tyro字段暴露为`--args.*`而在训练前退出，第二次启动在step 1遇到legacy dynamic while-loop不可反向求导；分别修复CLI入口和训练教师图为静态一步EAR/final endpoint后，第三次训练正常完成，含编译总耗时`221.35 s`，最佳验证checkpoint为step 500。Task8 HF `task_id=6`共得到188个anchor到anchor+10的有效pair，按episode划分为144 train、22 validation、22 test。held-out test上transport相对stale的action MSE ratio为`0.506951`、EAR为`0.608809`、IAR为`0.902244`、KV为`0.926226`，对应约49.3%、39.1%、9.8%和7.4%的gap reduction；adapter batch1延迟mean/p95为`0.270703/0.284699 ms`。预设的严格离线门因KV未达到20%且IAR略低于10%而为false，但下游action和EAR信号足够进入真实Task8快速门；risk head当前预测refresh rate为1.0、AUC为0.719，因此本轮不启用风险门控。sidecar位于`/root/autodl-tmp/acotvla/p3t_prefix_transport/task8_stride10_s7_500_dd24cfe/final/p3t_prefix_transport`，本条记录时尚未获得20局闭环成功率或端到端速度结果。

P3T-Lite随后完成Task8正式20局闭环，输出位于`/root/autodl-tmp/acotvla/p3t_prefix_transport/task8_formal20_p3t_s7_dd24cfe/eval`。协议固定initial-state IDs 10–29、seed7、H10、EAR/final NFE1和period2；最终`11/20`，成功IDs为10、15、16、18、19、20、21、23、25、26、28，失败IDs为11、12、13、14、17、22、24、27、29。1422次policy调用中720次fresh、702次transport，reuse rate为`49.37%`；policy/server/RPC分别为`57.8508/69.2432/70.9078 ms/call`，transport本身为`2.8156 ms/fast call`。排除大于500 ms的调用后，同运行fresh均值`65.6882 ms`、transport fast均值`49.8124 ms`，即约12% policy-compute节省，但transport比raw reuse fast的`46.9809 ms`多约2.83 ms。

与相同20个initial states的raw prefix reuse配对，P3T产生3次rescues（15、19、20）和3次regressions（14、24、27），总成功率仍同为`11/20`，没有质量净收益。fast-call risk在失败episode与成功episode中的均值分别为`0.73376`和`0.73817`，范围高度重叠且方向不支持危险调用拒绝，因此当前risk不能在保留加速的同时恢复质量。结论是整块深层KV的Action-CoT条件transport虽然离线缩小action/EAR误差并获得实际约12%计算节省，但不能控制接触任务的闭环漂移，P3T-KV版本判no-go；不再继续调其MSE、period或risk阈值。完成结果已通过现有飞书脚本发送并得到`FEISHU_P3T_RESULT_VERIFIED`，评测与policy服务已停止，GPU释放。
