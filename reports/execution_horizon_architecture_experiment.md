...
2026-09-08。用户授权完成三组实验；结构、特征接口、补录与训练脚本、三端点评测及调度器已实现，服务器84项针对性检查通过。下一步为真实模型下的360 root特征补录与三组闭环，尚无新成绩。
...
复用上一批300训练root（每任务IDs300–329）及60早停root（330–335）、各五H×五配对A-continuation标签。旧NPZ未保存完整prefix和expert hidden，因此从相同bank初态、环境seed及逐调用policy seed重放，到旧source_decision_index/root_step补录；仅在物理状态、环境动作、A输出及历史字段一致后复用标签。全部root都需通过；不按成功与否删样本或放宽门槛。原trial结果及原计时直接保留，补录导出耗时不覆盖旧RPC标签。

两组同seed7、Adam lr1e-4、batch64、固定650次更新，每25步及step0选模。只优化新增参数，A和VLA冻结。目标沿用`-E[paired success delta - 0.02 * paired RPC delta seconds] + 0.05 KL(pi_A || pi)`；按早停root等权greedy成功差优先、RPC差次优选择checkpoint，实际训练满650步。已有归一化输入原样使用。
...
入口：`collect_execution_horizon_architecture_features.py`、`train_execution_horizon_architecture.py`、`run_execution_horizon_architecture_experiment.py`。新评测模式为`ordered_visual_query`和`ordered_expert_hidden`，分别使用独立sidecar端点；原默认评测行为保持不变。
...
旧标签只衡量当前H改变后由A继续的局部回报，不包含新增模块未来开销；实际全程收益由闭环衡量。开发初态已用于前批评测，只有预留346–365仍未使用。冻结专家hidden可能与已有表示冗余，也不等于校准置信度；原始prefix含视觉与语言上下文，条件化pool不预设某个槽必然对应物体或接触区域。
