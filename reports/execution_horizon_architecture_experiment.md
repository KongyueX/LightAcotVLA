...
2026-09-08。三组实验已全部完成：本批A成功90/100，条件化视觉query为89/100，expert hidden为88/100，候选RPC分别增加4.86%和9.18%。无候选晋级，保留A，未运行预留测试；见[完整结果](execution_horizon_architecture_results.md)。
...
300训练root和60早停root共9000分支全部完成；两个模块各训练650步，query选step75，hidden选step0。三组各100局开发闭环完整结束，query救回5/退化6、hidden救回5/退化7；hidden结果对应零输出初始化，不能归因于学到hidden信息。采集及模型/评测代码8e0c0ad，训练与控制器5b70d1d。实验服务8050/8051/8052已关闭，原8040 A保持，数据和权重全部保留。
