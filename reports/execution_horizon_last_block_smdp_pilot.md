...
三轮已于2026-09-07 04:04完成，控制器exit0；没有新候选优于A，保留A，未启动条件final200。以下为相同official20–29、10tasks×10的开发验证，时间单位为每局秒。
...
| MC预热＋末层PPO Round2 | 92/100 | 17.15 | 1.850158 | 2.053689 | 11.830692 |
| Round2相对A | -1个百分点 | +7.66% | +11.56% | +12.04% | +8.49% |
| MC预热＋末层PPO Round3 | 92/100 | 17.16 | 1.841985 | 2.045421 | 11.669258 |
| Round3相对A | -1个百分点 | +7.72% | +11.06% | +11.59% | +7.01% |

Round3是本批新候选中按成功数/RPC排序最好的一个，但仍未达到相对A的准入条件。它较Round2补回Task2/episode24、Task5/episode26，同时新增Task0/episode27、Task6/episode20失败，成功数抵消；RPC再降0.40%、完整episode再降1.36%。三轮稳定救回A的Task6/episode21、24和Task8/episode22、27；三轮均退化的状态为Task3/episode23、Task5/episode27、Task9/episode24。

Round2较Round1补回Task8/episode26、Task9/episode27两局，未新增失败，RPC下降3.40%、完整episode下降1.04%。相对A仍为4rescues/5regressions，`eligible=false`；这是阶段性恢复，尚未超过初始A。

Round1相对A有4rescues/7regressions；净变化为Task6 +2、Task8 +1，Task2/3各-1，Task5/9各-2，其余任务不变，`eligible=false`。内部critic预测误差改善尚未转化为优于A的闭环成功率或时间收益。

第一轮验证1758个决策中，客户端selector耗时均值1.863ms/call、0.032745秒/局，发生在RPC计时结束之后，因此不能直接解释RPC增量；即使将其全部视作新增成本，也只相当于完整episode增量1.050374秒的3.12%。RPC单call均值115.067→120.932ms，调用数同时增加10.36%。按旧单价计算的额外调用成本为0.189860秒，另有单call单价变化0.103108秒，合计RPC增量0.292968秒；这是算术分解而非因果归因。当前结果支持继续关注闭环决策与轨迹变化，未显示客户端重算开销是主要原因。

在双方均成功的86局中，RPC仍增加9.65%、完整episode增加5.34%，因此均值变慢不只来自多失败3局。仿真、观测处理、缓存日志和跨运行差异未进一步分离。

Round1实际H5/10/15/20/25计数为167/60/177/555/799，Round2为159/51/184/571/750，Round3为188/91/151/570/716。H25选择多少本身不作为好坏结论，仍只按成功率和真实时间择优。三轮完整数据、checkpoint和日志均保留，不追加本批第4/5轮。
...
第二轮100条采集完整，随机训练轨迹94成功、平均RPC2.194491秒；该94/100与闭环greedy validation的92/100是不同指标。critic预热10epochs选epoch6，PPO4epochs全部接受，`actor_changed=true`、old-policy KL0.003331、无回退，总耗时83.40秒，eta由0.016065降到0.008010。checkpoint为`ACTIVE/experiment/round02/training/checkpoint`。

critic内部留出Brier从0.117407降到0.114967（-2.08%），同时训练Brier从0.112699降到0.051643（-54.18%）；留出联合目标0.448289→0.425462。说明本轮训练拟合增益明显强于留出增益，不足以认定价值预测已稳定，也不能把两局闭环恢复单独归因于预热。03:35时第三轮采集57/100，自动链继续运行。

## 第三轮与结论

第三轮随机采集100条、96成功，来自新训练IDs140–149，不能与前两轮87/94视作同cohort的成功率增长。MC预热选择epoch6，内部留出联合目标0.232461→0.218207，但success Brier由0.052921升到0.053746（+1.56%）；训练Brier0.053370→0.030895。PPO4epochs全部接受、112updates、old-policy KL0.003903、actor_changed=true，无回退，总75.32秒。三轮均有实际参数更新，不能将未胜出解释成训练未执行。

一个具体的目标设置问题是：固定3秒预算高于三轮随机采样RPC均值2.606506/2.194491/2.116348秒，导致eta从0.02依次降至0.016065/0.008010/0。最后一轮actor的`A_success − eta × A_cost`不再包含时间优势项；成本critic仍训练，但其梯度不进入actor。满足3秒预算不等于超过A的1.833秒greedy验证时间。

本批结论是MC预热与现有末层微调尚未带来优于A的闭环收益，停止继续加轮数。随后独立完成的[固定正成本权重小版本](execution_horizon_fixed_cost_smdp_pilot.md)从原A初始化与原首轮采样数据重训一次，使用`--dual-learning-rate 0`保留eta0.02；结果仍为90/100、RPC2.105222秒，未胜出。固定时间权重这一点没有解决成功率差距，两个批次均已结束，保留A，不扫描更多系数。
