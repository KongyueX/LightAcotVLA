...
本批结论是MC预热与现有末层微调尚未带来优于A的闭环收益。停止本批继续加轮数。下一项仅做[固定正成本权重的小版本](execution_horizon_fixed_cost_smdp_pilot.md)：从原A初始化及原首轮采样数据重训一次，使用已有`--dual-learning-rate 0`保留eta0.02，不改变架构、不重采、不扫描权重。它针对时间惩罚被自动撤销的设置，不保证解决成功率差距；若仍不胜出则结束该小版本。
