...
全部已启动实验完成，最终masked事件策略91/100对A93/100，RPC+7.08%、整局+2.84%；按预定停止条件保留A并结束该路线。汇总见[最终报告](../NARRATIVE_REPORT.md)。

## 最终第三阶段：固定masked事件策略的完整闭环（已完成）

第二轮67root/268分支完成。事件early的continue35/38，fresh和masked均36/38且相同6/19触发、RPC0.594490897秒；未满足fresh超过masked的原条件，没有启动fresh三组开发。第二次复盘6/10、not ready，建议只检验更窄的问题：不使用新proprio的缓存上下文事件调度是否改善A。

固定R2 masked step25 checkpoint，threshold=0.027104230597615242；不训练、不校准、不换特征或checkpoint。使用既有`ordered_midchunk_masked`执行路径，保留相同事件观察开销；决策中当前proprio及差值已遮蔽，不能用这次对照声称新proprio收益。

与A在已有开发IDs336–345、Task0–9各跑100局、交错共200局；这是本轮首次完整开发验证，但该cohort曾在项目其他开发中使用，不能称为全新独立测试。所有模型权重、阈值与协议在开跑前锁定，成功优先、真实RPC次优，同时报告policy/monitor/整局时间、事件检查和触发数、配对救回/退化。原fresh价值条件仍记为未通过，不回溯改写。若该固定策略结果不利，本窗口终止这条路线；预留346–365仍不使用。
